  ...
  <div class="node-wrap">
    <div class="node-wrap-box">
      <div>
        <div class="title" style="background:#ff943e;">
          <span>审批人</span>
          <Icon @click="delApp(workflowList.WorkFlowNodeModels,i)" type="md-close" color="#fff" size="14" class="close" />
        </div>
        <div class="content" @click="appUser(obj)">
          <div class="text">
            <span v-if="obj.ApprovalMemberList.length">
              <span v-for="(appItem,appIndex) in obj.ApprovalMemberList" :key="appIndex">{{appItem.FullName}}&nbsp;&nbsp;</span>
            </span>
            <span v-else style="color:#bfbfbf;">请设置审批人</span>
          </div>
    ...
    <div class="add-node-btn-box">
      <div class="add-node-btn">
        <button class="btn" @click="addNode(false,$event,workflowList,i,workflowList.WorkFlowNodeModels,i)">
...
<script>
  export default {
    ...
    methods: {
      ...
      // 弹出框//审批人/抄送人/条件节点
      addNode (bool, e, all, allIndex, val, index) {
        if (this.isBranch) {
          this.nodeOption = {
            bool: bool,
            eLeft: e.target.getBoundingClientRect().left,
            eTop: e.target.getBoundingClientRect().top,
            all: this.branchOption.branchItem,
            allIndex: this.branchOption.branchIndex,
            val: this.branchOption.branchChild,
            index: this.i
          }
        } else {
          this.nodeOption = {
      ...
      // 点击审批人节点
      appUser (val) {
        this.$store.commit('common/getAppUser', val)
        this.$store.commit('common/changeAppUserModal', true)
      }
    }
  }
</script>
