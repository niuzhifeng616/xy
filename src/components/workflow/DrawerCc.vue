        ...
        <div>
            <div class="app-user-box">
                ...
                <div class="app-user-list">
                    <div v-for="(item,index) in ccname" :key="index" v-cloak class="pr">
                        <div class="app-user-list-style nowrap" :title="item.FullName">
                            <span class="nowrap">{{item.FullName}}</span>
                        </div>
                        <Icon @click="delCcModal(index)" type="ios-close-circle" color="#f87c7c" size="22" class="cursor-p" style="position:absolute;top: -13px;right: 24px;"></Icon>
                    </div>
                    <div @click="addCcPep" class="app-user-list-style-dashed"><Icon type="md-add" color="#4196ff" size="22"></Icon></div>
                </div>
            </div>
        </div>
        <div class="work-flow-drawer">
            <Button class="xy-btn-primary" shape="circle" @click="ccUserBtn">保存</Button>
...
<script>
  ...
  export default {
    ...
    computed: {
      isShow: {
        get () {
          return this.$store.state.common.ccUserModal
        },
        set (newValue) {
          return newValue
        }
    ...
    watch: {
      isShow (newVal) {
        if (newVal) {
          this.clickCcUser(this.$store.state.common.ccUserOption)
        } else {
          this.$store.commit('common/getCcUser', {})
        }
      }
    },
    mounted () {
    ...
    methods: {
      ...
      selectConfirm (data) {
        if (data.length > 0) {
          ...
          //   data.map(item => {
          //     let index1 = this.ccname.map(item1 => {
          //       return item1.UserID
          //     }).indexOf(item.UserID)
          //     if (index1 === -1) {
          //       this.ccname.push({
          ...
          this.xy.msgError('抄送人不能为空。')
        }
      },
      close () {
        this.$store.commit('common/changeCcUserModal', false)
      },
      clickCcUser (val) {
        this.recursion = val.WorkFlowNodeID
        // 抄送人数组
        this.ccname = []
        val.CcMemberList.map(item => {
          this.ccname.push(item)
        })
      },
      delCcModal (i) {
        this.ccname.splice(i, 1)
      },
      addCcPep () {
        this.isTeacher = true
      },
      ccUserBtn () {
        if (this.ccname.length > 0) {
          if (this.ccname.length > 10) {
            this.xy.msgError('抄送人最多可添加10人。')
          } else {
            // 抄送人递归recursion
            this.ccUserRecursion(this.workflowList.WorkFlowNodeModels)
          }
        } else {
          this.xy.msgError('请先添加抄送人。')
        }
      },
      // 抄送人递归recursion
      ccUserRecursion (list) {
        if (list.length > 0) {
          for (var i = 0; i < list.length; i++) {
            if (this.recursion === list[i].WorkFlowNodeID) {
              list[i].CcMemberList = []
              for (var j = 0; j < this.ccname.length; j++) {
                list[i].CcMemberList.push(this.ccname[j])
              }
            } else {
              this.ccUserRecursion(list[i].WorkFlowNodeModels)
            }
          }
        }
        this.close()
