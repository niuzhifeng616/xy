<template>
<div>
    <div class="dingflow-design pr" ref="dingflowDesign" @scroll="listScroll">
        <div class="zoom">
        <div v-if="shrinkNum >= 0.6" class="zoom-out" @click="shrink">-</div>
        <div v-else class="zoom-out disabled">-</div>
        <span v-cloak>{{shrinkB}}%</span>
        <div v-if="shrinkNum <2" class="zoom-in" @click="enlarge">+</div>
        <div v-else class="zoom-in disabled">+</div>
        </div>
        <div class="ie-polyfill-container">
            <div ref="boxScale" class="box-scale" id="box-scale" style="transform: scale(1);transform-origin: 50% 0px 0px;">
                <div class="node-wrap">
                <div class="node-wrap-box start-node">
                    <div>
                    <div class="title" style="background:#576a95;"><span>发起人</span></div>
                    <div class="content">
                        <div class="text">由菜单权限设置</div>
                    </div>
                    </div>
                </div>
                <div class="add-node-btn-box">
                    <div class="add-node-btn">
                        <button class="btn" @click="addNode(true,$event,workflowList.WorkFlowNodeModels,0,workflowList.WorkFlowNodeModels,0)">
                            <Icon type="md-add" color="#fff" size="22" />
                        </button>
                    </div>
                </div>
                </div>
                <WorkFlow :workflowList="workflowList" @listenToChildScrollLeft="getScrollLeftNum"/>
                <div class="end-node"><div class="end-node-circle"></div><div class="end-node-text">流程结束</div></div>
                <AntPopover :workflowList="workflowList" :type="type"/>
            </div>
        </div>
    </div>
    <div class="worflow-submit">
        <Button class="xy-btn-primary" shape="circle" @click="workflowOk">发布</Button>
        <Button class="xy-btn-close"  @click="workflowCancel">取消</Button>
    </div>
    <DrawerApp :workflowList="workflowList"/>
    <DrawerCc :workflowList="workflowList"/>
    <DrawerCondition :workflowList="workflowList" :type="type"/>
</div>
</template>
<script>
  import WorkFlow from '@/components/workflow/WorkflowComponents.vue'
  import AntPopover from '@/components/workflow/AntPopover.vue'
  import DrawerApp from '@/components/workflow/DrawerApp.vue'
  import DrawerCc from '@/components/workflow/DrawerCc.vue'
  import DrawerCondition from '@/components/workflow/DrawerCondition.vue'
  export default {
    name: 'workflow',
    props: {
      type: Number,
    ...
    methods: {
      // 缩小
      shrink () {
        this.shrinkNum = this.shrinkNum - 0.1
        this.shrinkB = (this.shrinkNum * 100).toFixed(0)
        if (this.shrinkNum > 0.5) {
          this.$refs.boxScale.style.transform = 'scale(' + this.shrinkNum + ')'
        }
      },
      // 放大
      enlarge () {
        this.shrinkNum = this.shrinkNum + 0.1
        this.shrinkB = (this.shrinkNum * 100).toFixed(0)
        if (this.shrinkNum < 2.1) {
          this.$refs.boxScale.style.transform = 'scale(' + this.shrinkNum + ')'
        }
      },
      listScroll (e) {
        let str = [e.target.scrollLeft, e.target.scrollTop].join(',')
        this.$store.commit('common/setTargetXY', str)
      },
      getScrollLeftNum (val) {
        this.$nextTick(() => {
          this.$refs.dingflowDesign.scrollLeft = val + 'px'
        })
      },
      // 弹出框//审批人/抄送人/条件节点
      ...
      workflowOk () {
        this.isCcLength = true
        this.isAppLength = true
        this.isConLength = true
        // this.workflowObject.WorkFlowNodeModels = this.workflowList.WorkFlowNodeModels
        this.searchTureOrFalse(this.workflowList.WorkFlowNodeModels)
        if (!this.isCcLength) {
          this.xy.msgError('所有“抄送人”节点不能为空。')
          return false
        }
        if (!this.isAppLength) {
          this.xy.msgError('所有“审批人”节点不能为空。')
          return false
        }
        if (!this.isConLength) {
          this.xy.msgError('所有“条件”节点不能为空。')
          return false
        }
        if (this.isCcLength && this.isAppLength && this.isConLength) {
          this.$emit('listenWorkflow', true)
        }
      },
      // 递归查询是否所有节点都正确
      searchTureOrFalse (list) {
        if (list.length > 0) {
          list.map(item => {
            if (item.NodeType === 1) { // 抄送人
              if (item.CcMemberList.length === 0) {
                this.isCcLength = false
              }
            }
            if (item.NodeType === 2) { // 审批人
              if (item.ApprovalMemberList.length === 0) {
                this.isAppLength = false
              }
            }
            if (item.NodeType === 4) { // 条件
              if (item.WorkFlowConditions.length === 0) {
                this.isConLength = false
              }
            }
            this.searchTureOrFalse(item.WorkFlowNodeModels)
          })
        }
      },
      workflowCancel () {
