<template>
  <div>
    <div v-for="(item,i) in workflowList.WorkFlowNodeModels" :key="i">
      <cc-member v-if="item.NodeType===1" :workflowList=workflowList :ccItem=item :i=i :branch=isBranch :branchOption=branchOption></cc-member>
      <app-member v-if="item.NodeType===2" :workflowList=workflowList :appItem=item :i=i :branch=isBranch :branchOption=branchOption></app-member>
      <div class="branch-wrap" v-if="item.NodeType===3 && item.WorkFlowNodeModels.length>0">
        <div class="branch-box-wrap">
            <div class="branch-box">
                <button class="add-branch" @click="addBranchList(item)">添加条件</button>
                <div class="col-box" v-for="(child1,index) in item.WorkFlowNodeModels" :key="index" v-cloak>
                    <div class="top-left-cover-line" v-if="index===0"></div>
                    <div class="top-right-cover-line" v-if="index === item.WorkFlowNodeModels.length-1"></div>
                    <div class="bottom-left-cover-line" v-if="index===0"></div>
                    <div class="bottom-right-cover-line" v-if="index === item.WorkFlowNodeModels.length-1"></div>
                    <div class="condition-node">
                        <div class="condition-node-box">
                            <div class="auto-judge">
                                <div class="sort-left" v-if="index>0" @click="changePriorityLeft(item.WorkFlowNodeModels,index)">&lt;</div>
                                <div class="title-wrapper">
                                    <span class="editable-title" v-cloak>条件{{index+1}}</span>
                                    <span class="priority-title" v-cloak>优先级{{index+1}}</span>
                                </div>
                                <Icon @click="delCondition(item,workflowList,index)" type="ios-close" color="#f87c7c" size="26" class="close cursor-p" />
                                <div class="content" @click="addBranch(child1)" v-cloak>{{child1.WorkFlowNodeName?child1.WorkFlowNodeName:'请设置条件'}}</div>
                                <div class="sort-right" v-if="index<item.WorkFlowNodeModels.length-1" @click="changePriorityRight(item.WorkFlowNodeModels,index)">&gt;</div>
                            </div>
                            <div class="add-node-btn">
                                <button class="btn" @click="addNode(false,$event,item,index,child1,index)">
                                    <Icon type="md-add" color="#fff" size="22" />
                                </button>
                            </div>
                        </div>
                    </div>
                    <workflow-component :workflowList=child1 :branch=true :branchOption="{
                      branchItem:item,
                      branchIndex:index,
                      branchChild:child1.WorkFlowNodeModels
                    }"></workflow-component>
                </div>
            </div>
            <div class="add-node-btn-box">
                <div class="add-node-btn">
                    <button class="btn"
                        type="button"
                        @click="addNode(false,$event,workflowList,i,workflowList.WorkFlowNodeModels,i,3)">
                        <Icon type="md-add" color="#fff" size="22"></Icon>
                    </button>
                </div>
            </div>
        </div>
...
<script>
  import CcMember from '@/components/workflow/CcMember.vue'
  import AppMember from '@/components/workflow/AppMember.vue'
  export default {
    ...
    methods: {
      addBranch (val) {
        this.$store.commit('common/getCondition', val)
        this.$store.commit('common/changeConditionModal', true)
      },
      // 添加条件
      addBranchList (val) {
        this.isAntPopover = false
        if (val.WorkFlowNodeModels.length < 6) {
          val.WorkFlowNodeModels.push({
            ...
            ApprovalMode: 0,
            Level: val.WorkFlowNodeModels.length + 1,
            ParentID: 0,
          ...
          this.scrollLeftNum += 320
          this.$emit('listenToChildScrollLeft', this.scrollLeftNum)
        } else {
          this.xy.msgError('同级条件最多添加6个。')
        }
      },
      // 删除条件
      delCondition (val, parentVal, i) {
        if (val.WorkFlowNodeModels.length > 2) {
          val.WorkFlowNodeModels.splice(i, 1)
        } else {
          if (parentVal.length !== undefined) {
            this.workflowList[0].WorkFlowNodeModels = []
          } else {
            if (parentVal.NodeType === 4) {
              for (let i = 0; i < parentVal.WorkFlowNodeModels.length; i++) {
                if (parentVal.WorkFlowNodeModels[i].NodeType === 3) {
                  parentVal.WorkFlowNodeModels.splice(i, 1)
                }
              }
            } else {
              var isNode3 = parentVal.WorkFlowNodeModels.filter((item) => {
      ...
      // 调换优先级
      changePriorityLeft (arr, i) {
        arr[i - 1] = arr.splice(i, 1, arr[i - 1])[0]
        arr[i].Level = arr[i].Level + 1
        arr[i - 1].Level = arr[i - 1].Level - 1
      },
      changePriorityRight (arr, i) {
        arr[i] = arr.splice(i + 1, 1, arr[i])[0]
        arr[i].Level = arr[i].Level - 1
        arr[i + 1].Level = arr[i + 1].Level + 1
      },
      // 弹出框//审批人/抄送人/条件节点
      addNode (bool, e, all, allIndex, val, index, isThree) {
        this.nodeOption = {
          bool: bool,
