  ...
  <div class="xy-content-module">
    ...
    <div class="xy-content-body">
          ...
          >
            ...
            <div slot="CreateByDateRender" slot-scope="text, record">
               {{record.CreateByDate.slice(0, 16).replace('T',' ')}}
            </div>
    ...
    <Modal v-model="screenModel" class="screenModel" :title="createStatus ==='create' ? '创建方案' : '编辑方案'" width="500" :mask-closable="false" @on-visible-change="cancelHandle">
      <Form ref="caseForm" :model="caseForm" label-position="top" class="edit-form" :rules="caseRules">
        ...
        <FormItem label="起止日期" prop="date" labelFor="time">
    ...
    methods: {
      ...
      async getTableList () {
        ...
        if (rs.success) {
          ...
          this.pageList = rs.data.data !== null ? rs.data.data.length : 0
          this.data = rs.data.data !== null ? rs.data.data : []
      ...
      setDetail (row) {
        ...
        this.$router.push({
          path: '/xyHome/AccessSchoolGo/SetDetailGo',
      ...
      async editCase () {
        console.log(this.formateDate(this.caseForm.dateList[0]))
        let params = {
          SchoolAccessRecordSettingID: this.caseForm.caseId,
          SchoolAccessRecordSettingName: this.caseForm.caseName,
          SchoolDistrictId: this.caseForm.school,
          RoleID: this.caseForm.peopleType,
          StartDate: this.formateDate(this.caseForm.dateList[0]),
          EndDate: this.formateDate(this.caseForm.dateList[1])
        }
