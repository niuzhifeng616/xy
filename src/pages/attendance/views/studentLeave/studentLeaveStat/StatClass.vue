        ...
        <div v-if="!isType && !rightEmpty">
            <XyAntTable
              ...
              <div slot="FullName" slot-scope="text, record">
                <span class="table-el-hover" @click="toStatPeople(record)">
                  {{record.FullName}}
                </span>
              </div>
              ...
              <span slot="operation" slot-scope="text, record">
                <a @click="toStatPeople(record)">详情</a>
              </span>
            </XyAntTable>
        </div>
        <div v-if="isType  && !rightEmpty">
            ...
            >
              ...
              <div slot="UserName" style="min-width:120px" slot-scope="text, record">
               ...
               <div slot="ClassName" style="min-width:150px" slot-scope="text, record">
                  {{record.ClassName}}
              </div>
              <div slot="LeaveTypeName" style="min-width:100px" slot-scope="text, record">
                  {{record.LeaveTypeName}}
              </div>
              <div slot="ApplicationName" style="min-width:100px" slot-scope="text, record">
                  {{record.ApplicationName}}
              </div>
              <div slot="ApplicantName" style="min-width:120px" slot-scope="text, record">
                  {{record.ApplicantName}}
              </div>
              <div slot="CreateTime" style="min-width:200px" slot-scope="text, record">
                  {{record.CreateTime}}
              </div>
  ...
  export default {
    ...
    methods: {
      ...
      getTP: function () {
        ...
        this.xy.get(
          `${this.$store.state.apiPath}/api/StudentLeaveStatistics/GetPageForClass`,
          {
            SchoolDistrictID: _this.schoolDistrictID,
            GradeYearID: _this.gradeYearID,
            ClassID: _this.classID,
          ...
          if (res.success) {
            ...
            for (let i in _this.getColList) {
              _this.columnsClass.push({
            ...
            _this.columnsClassData = res.data.LeaveSummaryDetail
          } else {
            this.xy.msgError(res.msg)
          }
          this.tableLoading = false
      ...
      backLevel: function () {
        ...
        this.$parent.comName = 'StatGrade'
      },
      // 点击详情跳转到班级页面
      toStatPeople: function (row) {
        this.$parent.defaultValue.push(Number(row.ID))
        this.$parent.cascaderData = 4
        this.$parent.comName = 'StatPeople'
        this.$parent.studentID = row.ID.toString()
        this.$parent.studentName = row.Name
      },
      // 前一天/周/月
      before: function () {
        this.isType = false
      ...
      exportModal: function () {
        if (this.total !== 0) {
          ...
          if (this.isType) {
            ...
            _this.xy.downFile(
              `${_this.$store.state.apiPath}/api/StudentLeaveStatistics/ExportForClass`,
              {
                SchoolDistrictID: _this.schoolDistrictID,
                GradeYearID: _this.gradeYearID,
                ClassID: _this.classID,
