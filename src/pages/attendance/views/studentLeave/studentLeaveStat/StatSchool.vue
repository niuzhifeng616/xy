  ...
  <div class="xy-content-body">
    ...
    <div>
      ...
      <div class="leaveBody" v-if="!leftEmpty || !rightEmpty">
        <div v-if="leftEmpty" class="no-data-box" v-cloak style="margin: 100px auto;">
          <img class="no-data-img" src="@/assets/common/nullData.svg" />
          <span class="no-data-text" v-cloak>该搜索条件下没有结果。</span>
        </div>
          ...
          <div v-if="!isType && !rightEmpty">
            ...
            >
              <div slot="Name" slot-scope="text, record">
                <span class="table-el-hover" @click="toStatGrade(record)">
                  {{record.Name}}
                </span>
              </div>
              ...
              <span slot="operation" slot-scope="text, record">
                <a @click="toStatGrade(record)">详情</a>
              </span>
            </XyAntTable>
          </div>
          <div v-if="isType  && !rightEmpty">
    ...
    methods: {
      ...
      itemDetail (item) {
        ...
        this.xy.get(`${this.$store.state.apiPath}/api/StudentLeaveStatistics/GetStudentDetailByTypeSummary`, {
          ...
          StartTime: _this.startDate,
          EndTime: _this.endDate,
          // PageIndex: _this.pageDetailCurrent,
          ...
          if (res.success) {
            ...
            _this.pagination1.total = res.data.totalRecords
            _this.columnsDetail = [
              {
                title: '',
                dataIndex: 'Icon',
      ...
      getTP () {
        ...
        this.applicationSubMenuID = idArr[2]
        _this.xy.get(
          `${_this.$store.state.apiPath}/api/StudentLeaveStatistics/GetPageForSchool`,
          {
            SchoolDistrictID: _this.schoolDistrictID,
            ApplicationID: _this.applicationID,
            ApplicationSubMenuID: _this.applicationSubMenuID,
          ...
          if (res.success) {
            ...
            _this.columnsCampusData = res.data.LeaveSummaryDetail
          } else {
            this.xy.msgError(res.msg)
          }
          this.tableLoading = false
      ...
      toStatGrade (row) {
        ...
        this.$parent.gradeYearID = row.ID.toString()
        this.$parent.gradeName = row.Name
      },
      // 前一天/周/月
      before () {
        this.isType = false
      ...
      exportModal () {
        if (this.total !== 0) {
          ...
          if (this.isType) {
            ...
            _this.xy.downFile(
              `${_this.$store.state.apiPath}/api/StudentLeaveStatistics/ExportLeaveForSchool`,
              {
                SchoolDistrictID: _this.schoolDistrictID,
                ApplicationID: _this.applicationID,
                ApplicationSubMenuID: _this.applicationSubMenuID,
