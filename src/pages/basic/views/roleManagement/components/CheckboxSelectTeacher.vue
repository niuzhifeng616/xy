...
<script>
  export default {
    ...
    props: {
      ...
      // 复杂业务中(例如：角色管理-添加教师)，选择教师点击‘下一步’后，再次点击‘上一步’返回选择教师时的反选数据。（此情况也不需要反选列表中已选的数据,setFilter传入的true）
      selectedPart: {
        type: Array,
        required: false
      },
      ...
      isShow (val) {
        if (val) {
          ...
          if (this.selectedPart && this.selectedPart.length > 0) {
            this.selectedPartIds = this.selectedPart.map(item => item.UserID)
          } else {
          ...
          for (let i = 0; i < orglist.Members.length; i++) {
            if (this.setFilter) { // 过滤列表中已选择的教师
              if (this.selectTeacherIds.indexOf(orglist.Members[i].UserID) === -1) { // 已经添加过的教师过滤不显示
                ...
                // 选择教师点击‘下一步’后，再次点击‘上一步’返回选择教师时的反选数据。
                if (this.selectedPartIds.length > 0) {
                  // 右侧教师多个过滤操作，如果，有多个同一教师只显示一个
                  if (this.selectedPartIds && this.selectedPartIds.length && ((!this.checkAllList.length && this.selectedPartIds.includes(obj.UserID)) || (!this.checkAllList.map(v => `${v.title}_${v.sex}`).includes(`${obj.title}_${obj.sex}`) && this.selectedPartIds.includes(obj.UserID)))) {
                    this.checkAllList.push(obj)
                  }
                  // 返回‘上一步’的时候，将之前选中过的教师在左侧tree上面选中
                  if (this.selectedPartIds.includes(obj.UserID) && !this.curNode.includes(obj.id)) {
