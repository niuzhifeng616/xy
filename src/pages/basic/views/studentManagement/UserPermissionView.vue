          ...
          <div class="child-list" :key="child1.Name + index1" v-for="(child1, index1) in item.applications">
            ...
            <div class="child-flex clearfix">
              <div v-for="(child2, index2) in child1.subMenu" :key="child2.Name + index2" class="fl">
                <div class="operation-name f14 child2-name" :class="child2.PermissionArr.length>0?'active':'no-active'">
                  <b>{{child2.Name}}</b>
                </div>
                      ...
                      <div v-if="child3.OperChilds.length>0">
                        <div v-for="(child4, indxs) in child3.OperChilds" :key="child4 + indxs">
                          <div
                            ...
                            :class="child4.OperateDetail>0?'active':'no-active'"
                            v-if="child4.PermissionArr != null"
                            >
                            {{child4.OperationPermissionName}}
                          </div>
                          <div
                            v-if="child4.PermissionArr.length>0"
                            class="f12"
                            style="color:#DDDEE0;"
                            >
                            <span>来自：</span>
