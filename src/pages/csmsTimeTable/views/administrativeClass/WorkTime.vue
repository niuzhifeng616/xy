...
<div class="xy-content-body">
    <div style="margin-bottom:10px;">
        <div class="clearfix">
            <div class="fl">
                <Button class="xy-primary" :disabled="StepsHeader.isOperation != 0" @click="quicklySet" >快速复制</Button>
                <Button class="xy-info"  :disabled="StepsHeader.isOperation != 0" @click="quicklySetting">快速设定</Button>
                <Button class="xy-danger"  :disabled="StepsHeader.isOperation != 0" @click="clearAllTime" >清除时间</Button>
            </div>
            <div class="fr wt">
    ...
    <table id="tbClassTableTime" class="class-hour-table" style="width:100%;">
        <thead>
            <tr>
                <th width="8%">上课时间</th>
                <th width="16%">课节</th>
                <th class="weekselect" v-for="(week,index) in weekList" :key="index" :name="index" width="11%">
                    <span v-cloak>{{week.text}}</span>
                </th>
            </tr>
        ...
        <tbody>
            <tr v-for="(timeTable,index) in timeTableData" :key="index" >
                <td class="setTime" @click="trSetTime(timeTable)">
                    <span>设定</span>
                </td>
                <td>
                    <span v-cloak>{{timeTable.timeName}}</span>
                    <!-- 自习 添加-->
                    <span class="section-icon" v-if="(timeTable.type=='MS'||timeTable.type=='ES')&&timeTable.isLast">
                      <i class="iconfont add-up" @click="addSection(timeTable.type)">&#xe6d7;</i>
                      <span class="timer-text">自习</span>
                        <i class="iconfont  add-down" @click="reduce(timeTable.type,index)">&#xe6d0;</i>
                    </span>
                    <!-- 上午 添加-->
                    <span class="section-icon" v-if="timeTable.type=='AM'&&timeTable.isLast">
                        <i class="iconfont add-up" @click="addSection(timeTable.type)">&#xe6d7;</i>
                    ...
                    <span class="section-icon" v-if="timeTable.type=='AB' || timeTable.type=='PB'">
                        <Icon type="md-arrow-up arrow-up"  @click="upMove(timeTable.type,index)"/>
                        <span class="timer-text">课间</span>
                        <Icon type="md-arrow-down red-section arrow-down" @click="downMove(timeTable.type,index)"/>
                    </span>
                    <!-- 下午 添加-->
                    <span class="section-icon" v-if="timeTable.type=='PM'&&timeTable.isLast">
                        <i class="iconfont add-up" @click="addSection(timeTable.type)">&#xe6d7;</i>
                        <span class="timer-text">下午</span>
                         <i class="iconfont  add-down" @click="reduce(timeTable.type,index)">&#xe6d0;</i>
                    </span>
                </td>
                <td v-for="(time,index) in timeTable.weekTime"
                    :key="index"
    ...
    <div style="text-align: center;margin-top:15px;">
        <Button class="xy-btn-primary" shape="circle" :disabled="StepsHeader.isOperation != 0"  @click="saveTimeSubmit(0)" >保存</Button>
    </div>
        ...
        <div style="font-size:14px;">
            ...
            <div class="set-section" v-if="isShowMS">
                ...
                <div>
                    ...
                    <span>每节课时长:</span>
                    <Input clearable v-model="MSLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                    <span>分钟</span>
                </div>
            </div>
            <!-- 上午 -->
            <div class="set-section" v-if="isShowAM">
                ...
                <div>
                    ...
                    style="margin:0 10px"
                                 ...
                                 :editable="false"
                                 placeholder="请选择上课时间"
                                   class="time-picker"></TimePicker>
                    <span>每节课时长:</span>
                    <Input clearable v-model="AMLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                    <span>分钟</span>
                    <span v-if="isShowAB">
                        <span style="margin-left:10px">大课间时长:</span>
                        <Input clearable v-model="ABRecessLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                        <span>分钟</span>
                    </span>
                </div>
            </div>
                ...
                <div>
                    ...
                    style="margin:0 10px"
                                 ...
                                 placeholder="请选择上课时间"
                                   class="time-picker"></TimePicker>
                    <span>每节课时长:</span>
                    <Input clearable v-model="PMLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                    <span>分钟</span>
                    <span v-if="isShowPB">
                        <span style="margin-left:10px">大课间时长:</span>
                        <Input clearable v-model="PBRecessLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                        <span>分钟</span>
                    </span>
                </div>
            </div>
            ...
            <div class="set-section" v-if="isShowES">
                ...
                <div>
                    ...
                    <span>
                        <span>每节课时长:</span>
                        <Input clearable v-model="ESLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                        <span>分钟</span>
                    </span>
                </div>
            </div>
            ...
            <div class="set-section">
                <span>普通课间时长:</span>
                <Input clearable v-model="commonLength" style="width: 100px;margin:0 10px" :autofocus="true" />
                <span>分钟</span>
            </div>
        </div>
        <div slot="footer">
    ...
    <Modal v-model="modalSetTime"
        ...
        <div style="font-size:14px;">
            <!--<Radio -group v-model="timeRadio" @on-change="radioChange" style="margin-bottom:20px;">
                <Radio  label="0">
                    <span class="f14">开始设定</span>
                </Radio >
                <Radio  label="1">
                    <span class="f14">清空</span>
                </Radio >
            </Radio -group>-->
    ...
    <Modal v-model="modalQuicklySet"
        ...
        <div style="font-size:14px;max-height:350px;overflow-y:auto;">
            ...
            <ul class="quickly-set">
                <li>
                    <h2>行政班排课方案</h2>
                      <div  v-for="general in quicklySetData.General"
                           :key="general.Name">
                        <Radio  v-if="quicklySetData.General.length > 0"
                ...
                <li>
                    <h2>走班排课方案</h2>
                    <div v-for="moveClass in quicklySetData.MoveClass"
                        :key="moveClass.Name">
                        <Radio  v-if="quicklySetData.MoveClass.length > 0"
                ...
                <li>
                    <h2>校本排课方案</h2>
                    <div v-for="schoolBased in quicklySetData.SchoolBased"
                           :key="schoolBased.Name">
                        <Radio  v-if="quicklySetData.SchoolBased.length > 0"
                           v-model="schoolBased.ischecked"
                           @on-change="radioChanage(schoolBased)">
                           {{schoolBased.Name}}
                        </Radio >
                    </div>
    ...
    methods: {
      ...
      async saveTimeSubmit (val) {
        if (this.StepsHeader.isOperation === 0) {
          ...
          this.xy.loading()
          let res = await this.xy.post(`${this.$store.state.apiPath}/api/ClassTableTime/Create`, {
            ClassTableID: this.classPlan.classTableID,
            Models: models1
          })
          this.xy.unloading()
          if (res.success) {
            ...
            if (val === 0) {
              this.xy.msgSuc('保存成功。')
              this.$parent.liData.map((item) => {
                item.isActive = false
                return item
              })
              this.$parent.liData[2].isActive = true
              this.$store.commit('common/getParam', { pro: 'administrative', step: 2 })// 行政班排课
              this.$router.push({ path: '/xyhome/administrativeStepHeader/administrativeClassList', query: this.$store.state.common.menuInfo })
      ...
      async GetSettingList () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ClassTableTime/GetSettingList`, {
          classTableID: this.classPlan.classTableID
        })
        this.xy.unloading()
        if (res.success) {
      ...
      checkChange (val) {
        ...
        if (val === 'PM') {
          if (this.PMcheck) {
            ...
            this.timeTableData = this.timeTableData.filter(function (item) { return (item.type !== 'PM' && item.type !== 'PB' && item.type !== 'NS') })
            this.PBcheck = false
            this.NScheck = false
            this.PBdisabled = true
            this.NSdisabled = true
        ...
        if (val === 'AB') { // 上午大课间的添加删除
          if (this.ABcheck) {
            let AMList = this.timeTableData.filter(function (item) { return item.type === 'AM' })
            if (AMList.length > 1) {
              let contrastOrder = 1
              if (AMList.length > 2) {
                contrastOrder = 2
      ...
      addSection (type) {
          ...
          if (AMList.length < 6) {
            ...
            for (let j = 0; j < this.timeTableData.length; j++) {
              if (this.timeTableData[j].type === 'PM') {
                ...
                this.timeTableData[j].weekTime.map(function (item) {
        ...
        if (type === 'ES') { // 晚自习
          ...
          if (ESList.length < 4) {
            this.timeTableData.map(function (item) {
              if (item.type === 'ES') {
                item.isLast = false
              }
              return item
      ...
      upMove (type, index) {
        ...
        if (type === 'PB') { // 下午大课间
          ...
          if (PMList.length === 2) {
            let PBCopy = this.timeTableData.filter(function (item) { return item.type === 'PB' })
            this.timeTableData.splice(index, 1)
      ...
      downMove (type, index) {
        if (type === 'AB') { // 上午大课间
          ...
          if (AMList.length === 2) {
            let ABCopy = this.timeTableData.filter(function (item) { return item.type === 'AB' })
            this.timeTableData.splice(index, 1)
            this.timeTableData.splice(index + 1, 0, ABCopy[0])
          } else {
            this.xy.msgError('上午大课间必须在上午课节之间。')
        ...
        if (type === 'PB') { // 下午大课间
          let nextTwo = this.timeTableData.slice(index + 1, index + 3)
          let PMList = nextTwo.filter(function (item) { return item.type === 'PM' })
          if (PMList.length === 2) {
            let PBCopy = this.timeTableData.filter(function (item) { return item.type === 'PB' })
            this.timeTableData.splice(index, 1)
            this.timeTableData.splice(index + 1, 0, PBCopy[0])
      ...
      async ClearTime () {
        this.xy.loading()
        let res = await this.xy.post(`${this.$store.state.apiPath}/api/ClassTableTime/ClearTime`, {
          id: this.classPlan.classTableID
        })
        this.xy.unloading()
        if (res.success) {
          this.xy.msgSuc('清除全部时间成功。')
          this.MScheck = false
          this.AMcheck = false
          this.ABcheck = false
          this.ABdisabled = false
      ...
      async submitQuicklySet () {
        ...
        this.xy.loading()
        let res = await this.xy.post(`${this.$store.state.apiPath}/api/ClassTableTime/Setting`, {
        ...
        for (let i = 0; i < this.timeTableData.length; i++) {
          if (this.timeTableData[i].type === 'MS') { // 早自习
            if (MSStartTime === '') {
              ...
              MSStartTime = this.xy.moment(this.xy.timeFormat(MSStartTime)).add(Number(this.MSLength) + Number(this.commonLength), 'm').format('HH:mm')
            }
            MSEndTime = this.xy.moment(this.xy.timeFormat(MSStartTime)).add(Number(this.MSLength), 'm').format('HH:mm')
          ...
          if (this.timeTableData[i].type === 'AM') { // 上午
            if (AMStartTime === '') {
              ...
              if (this.timeTableData[i - 1].type !== 'AB') {
                AMStartTime = this.xy.moment(this.xy.timeFormat(AMStartTime)).add(Number(this.AMLength) + Number(this.commonLength), 'm').format('HH:mm')
              }
            }
            AMEndTime = this.xy.moment(this.xy.timeFormat(AMStartTime)).add(Number(this.AMLength), 'm').format('HH:mm')
            this.timeTableData[i].weekTime.map((item, index) => {
          ...
          if (this.timeTableData[i].type === 'AB') { // 上午大课间
            ...
            ABEdnTime = this.xy.moment(this.xy.timeFormat(ABStartTime)).add(Number(this.ABRecessLength), 'm').format('HH:mm')
          ...
          if (this.timeTableData[i].type === 'PM') { // 下午
            if (PMStartTime === '') {
              ...
              if (this.timeTableData[i - 1].type !== 'PB') {
                PMStartTime = this.xy.moment(this.xy.timeFormat(PMStartTime)).add(Number(this.PMLength) + Number(this.commonLength), 'm').format('HH:mm')
              }
            }
            PMEndTime = this.xy.moment(this.xy.timeFormat(PMStartTime)).add(Number(this.PMLength), 'm').format('HH:mm')
            this.timeTableData[i].weekTime.map((item, index) => {
          ...
          if (this.timeTableData[i].type === 'PB') { // 下午大课间
            ...
            PBEdnTime = this.xy.moment(this.xy.timeFormat(PBStartTime)).add(Number(this.PBRecessLength), 'm').format('HH:mm')
          ...
          if (this.timeTableData[i].type === 'ES') { // 晚自习
            if (ESStartTime === '') {
              ...
              ESStartTime = this.xy.moment(this.xy.timeFormat(ESStartTime)).add(Number(this.ESLength) + Number(this.commonLength), 'm').format('HH:mm')
            }
            ESEndTime = this.xy.moment(this.xy.timeFormat(ESStartTime)).add(Number(this.ESLength), 'm').format('HH:mm')
            this.timeTableData[i].weekTime.map((item, index) => {
