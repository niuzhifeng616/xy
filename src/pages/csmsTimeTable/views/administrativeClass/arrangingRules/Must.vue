          ...
          <table class="ban-must-table">
              <thead>
                  <tr>
                      ...
                      <th width="12%" v-for="week in weekData" :key="week.name" @click="weekDay(week)" style="min-width:100px">{{week.name}}</th>
                  </tr>
              </thead>
              <tbody>
    ...
    methods: {
      ...
      clearSetting: function () {
        if (this.StepsHeader.isOperation === 0) {
          this.xy.confirm('温馨提示', '您确定要全部清除必须排课时间设置吗？', this.clearSettingOk)
        }
      },
      clearSettingOk () {
        this.xy.loading()
