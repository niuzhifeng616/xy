      ...
      <table class="ban-must-table">
        ...
        <tbody>
          <tr v-for="schedule in scheduleData" :key="schedule.sectionName">
            ...
            <!-- <td v-for="(week,n) in schedule.weekList" :key="n"
              @click="week.isActive = !week.isActive" :class="week.isActive?'bg-red':''">
              <span v-if="week.isActive" style="color:#FFF;">禁止排课</span>
            </td> -->
          </tr>
        </tbody>
      </table>
          ...
          for (let i = 0; i < dataList.length; i++) {
            ...
            for (let j = 0; j < dataList[i].children.length; j++) {
              this.crouseTree[i].children.push({
                ...
                prohibitCount: dataList[i].children[j].ProhibitCount,
                checked: false,
                indeterminate: false,
                children: [],
                render: (h, func) => {
                  return h('span', {
                    ...
                    h('span', {
                      style: {
                        display: func.data.prohibitCount > 0 ? 'inline-block' : 'none',
                        marginLeft: '6px'
                      }
                    }, [
                      h('Icon', {
