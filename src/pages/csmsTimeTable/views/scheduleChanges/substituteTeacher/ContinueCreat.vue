    ...
    <div class="xy-content-title">
      <div>
        ...
        <i-select v-model="searchTeacher"
                  ...
                  placeholder="查找教师姓名"
                  style="width:250px;">
          <i-option v-for="(option, index) in optionList"
                    :data-aa="option.UserID"
                    :value="option.UserID"
                    :key='index'
          ...
          <table class="table"
            ...
            <tbody>
              <tr v-for="(time,Findex) in tableTimes"
                  :key='Findex'>
                <td >{{time.AMorPMOrderName}}</td>
                <td v-for="(td,Kindex) in 7"
                    :key='Kindex'>
                  <template v-for="(data,cindex) in leftBottomData">
                    <div class="subject"
                         :key='cindex'
                         v-if="data.AMorPM == time.AMorPM && data.AMorPMOrder == time.AMorPMOrder && data.Day == td-1">
                      <h5 >{{data.SubjectName}}</h5>
          ...
          <div class="right-substitute-teacher">
            ...
            <section v-show="curTab==='4'">
              ...
              <div class="tap-content">
                <ul class="teacher-ul"
                    v-if="rightTeacherData.length > 0">
                  <template v-for="(teacher,jIndex) in rightTeacherData">
                    <li @click="rigthTeacher($event,teacher)"
                        :key='jIndex'
                        :class="teacher.IsConflict?'no-chose-teacher':''"
...
<script>
  export default {
    ...
    // this.weekCur = this.$route.query.sourceWeek
    },
    mounted: function () {
      this.getTeachers()
    },
    methods: {
      ...
      async getSubstituteInfo () {
        let that = this
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/SubstituteCourseAdjustmentCourse/GetSubstituteInfo`, {
          classTableGenerateForSubstituteTeacherID: that.classTableGenerateForSubstituteTeacherID
        })
        if (res.success) {
          let resData = res.data
          that.weekCur = resData.Week.toString()
          that.searchTeacher = resData.SourceTeacherID
          that.teacherName = resData.SourceTeacherName
          that.sectionCur = resData.EffectiveWeek
          that.recordData = []
      ...
      // 周次改变
      weekChange: function (data) {
        let index = this.weekSelectData.map(function (item) { return item.key }).indexOf(data)
        this.weekData = []
        for (let i = 0; i < 7; i++) {
          this.weekData.push({
      ...
      async getWeek () {
        ...
        if (res.success) {
          ...
          if (that.weekCur === 0) {
            that.weekCur = that.weekSelectData[0].key
            ...
            let index = that.weekSelectData.map(function (item) { return item.key }).indexOf(that.weekCur)
            if (index !== -1) {
              that.weekDateName = that.weekSelectData[index].name
              that.weekData = []
              for (let i = 0; i < 7; i++) {
                that.weekData.push({
                  date: this.dateFun.format(this.dateFun.add(that.weekSelectData[index].val, 'd', i), 'MM.dd'),
                  weekName: this.xy.getChinesesWeekForNumSmall(i),
                  Date: this.dateFun.format(this.dateFun.add(that.weekSelectData[index].val, 'd', i), 'yyyy-MM-dd')
                })
              };
            }
      ...
      async rightSub () {
        ...
        let postData = {
          ...
          SourceTeacherID: this.searchTeacher,
          Week: this.weekCur,
      ...
      async clearRecord (item, index, tag) {
        let that = this
        if (tag === 1) {
          // if (index + 1 == that.recordData.length) {
          //    this.xy.msgError("只撤销被点击记录之后的记录，您点击的是最后一条记录，无可供撤销的记录。");
          // } else {
          this.xy.loading()
          ...
          if (res.success) {
            this.xy.msgSuc('清除成功。')
            $('.subject').removeClass('is-active')
            // let num = that.recordData.length - index - 1;
            // that.recordData.splice(index + 1, num);
      ...
      async cancel () {
        ...
        this.$Modal.confirm({
          ...
          async onOk () {
            this.xy.loading()
            let res = await
              that.xy.post(`${that.$store.state.apiPath}/api/SubstituteCourseAdjustmentCourse/Delete`, {
                id: that.classTableGenerateForSubstituteTeacherID
              })
            if (res.success) {
              that.xy.msgSuc('取消成功。')
      ...
      async completeSub () {
        ...
        let res = await this.xy.post(`${this.$store.state.apiPath}/api/SubstituteCourseAdjustmentCourse/CompleteSubstituteTeacher?classTableGenerateForSubstituteTeacherID=` +
          ...
          '&sourceTeacherID=' + that.searchTeacher +
          '&sourceTeacherID=' + that.searchTeacher)
        if (res.success) {
          that.getNotifyInfo()
        }
