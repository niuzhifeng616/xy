        ...
        <div class="clearfix">
          <div class="fl">
            <Button class="xy-primary"
                    @click="creatHoliday">创建放假</Button>
          ...
          <Alert show-icon
                 class="fl"
                 closable
                 style="margin:0 0 0 10px;">设置临时性的放假或补课。设为放假的日期将删除教师和学生的课表。设为补课的日期将为教师和学生生成课表。</Alert>
        </div>
      </div>
            ...
            <li class="temporary-holiday-li"
              ...
              <div class="top">
                ...
                <div class="content">
                  ...
                  <p class="time-text">放假日期：
                    <span>{{item.HolidayDate}}</span>
                  </p>
                  <p class="time-text">补回日期：
                    <span>{{item.BackupDate}}</span>
                  </p>
                </div>
              </div>
              <div class="bottom">
  ...
  export default {
    ...
    methods: {
      async getClassTable () {
        ...
        this.xy.loading()
        let res = await
          this.xy.get(`${this.$store.state.apiPath}/api/TemporaryHoliday/GetPage`, {
            type: 0,
      ...
      creatHoliday: function () {
        ...
        this.$router.push({ path: '/xyHome/creatHoliday', query: this.$store.state.common.menuInfo })
      },
      //
      look: function (row) {
        this.$store.commit('common/getThreeMenuName', '查看假期')
        this.$store.commit('common/getParam', { holidayBaseID: row.HolidayBaseID })
        this.$router.push({ path: '/xyHome/lookHoliday', query: this.$store.state.common.menuInfo })
      },
      // 冲突校验
