<template>
  <div class="xy-content-module" ref="Schedule">
    ...
    <Button class="export-btn xy-primary" @click="modalExport = true">导出</Button>
            ...
            <div v-for="(item,index) in subjectData" :key='index' style="margin-top:15px;">
              ...
              <CheckboxGroup class="teacher-box" v-model="item.selectSubject" @on-change="subjectChange(item,index)">
                <template v-for="(subject,cindex) in item.Orgs">
                  <Checkbox class="teacher-lable" :label="subject.StudentOrganizationID" :key="cindex">
                    ...
                    <td v-cloak :key="index" v-if="(week.val!=5&&week.val!=6)||isContainWeekend">
                      <div class="td-min">
                        <template v-for="(crouse,index) in item.SloverResultList">
                          <div v-cloak class="attend-cont-tit" :key="index" :style="{'background': crouse.bg,'borderLeftColor':crouse.color,'color':crouse.color}" v-if="crouse.Day == week.val  && crouse.AMorPM == sectionTd.AMorPM && crouse.AMorPMOrder == sectionTd.AMorPMOrder">
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        ...
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetSloverResultAdministrativeClassTimes`, {
          classTableIDs: this.classTableIDs,
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetSloverResultAdministrativeClassTimesClass`, {
          classTableIDs: this.classTableIDs
        })
        if (res.success) {
          this.xy.unloading()
      ...
      exportFile () {
        ...
        if (this.subExport === 'export') {
          // 导出 导出版
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportSloverResultAdministrativeClassTimes`, {
            classTableIDs: this.classTableIDs,
            // studentGradeForStudentYearID: this.studentGradeForStudentYearID,
            studentOrganizationID: this.studentOrganizationID,
          ...
          // 导出 打印版
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportPrintSloverResultAdministrativeClassTimes`, {
            classTableIDs: this.classTableIDs,
            // studentGradeForStudentYearID: this.studentGradeForStudentYearID,
            studentOrganizationID: this.studentOrganizationID,
            isContainWeekend: this.isContainWeekend,
      ...
      printLoad () {
        ...
        let routeUrl = this.$router.resolve({
          path: '/xyHome/semesterPrintAdministrative',
