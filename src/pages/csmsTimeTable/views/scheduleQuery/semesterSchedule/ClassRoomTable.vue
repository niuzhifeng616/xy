        ...
        <div class="left-options">
          ...
          <div class="options-body">
            <div class="search-opt">
             ...
             <Checkbox label="0" v-model="allTeacher" class="schedule-subject administrative-subject" @on-change="changeAllTeacher" :disabled="leftSearchName?true:false" v-show='subjectData.length>0'>全部场室</Checkbox>
            <div v-for="(item,index) in subjectData" :key='index' style="margin-top:15px;">
              ...
              <CheckboxGroup class="teacher-box" v-model="item.selectSubject" @on-change="subjectChange(item,index)">
                <template v-for="(subject,cindex) in item.HouseList">
                  <Checkbox class="teacher-lable" :label="subject.HouseID" :key="cindex" v-if="subject.HouseName.indexOf(leftSearchName) >= 0">
                ...
                <table id="right-table2" class="query-table horizontal-version">
                  <tbody>
                    <tr v-for="(item,index) in datatable" :key="index" ref="trListRight">
                      <template v-cloak v-for="(sectionTd,cindex) in sectionData">
                        <td :key="cindex" v-if="(sectionTd.Day!=5&&sectionTd.Day!=6)||isContainWeekend">
                          <div class="td-min">
              ...
              <table class="query-table">
                ...
                <tbody>
                  <tr v-for="(sectionTd,sindex) in festList" :key="sindex">
                    ...
                    <template v-for="(week,index) in weekData">
                      <td v-cloak :key="index" v-if="(week.val!=5&&week.val!=6)||isContainWeekend">
                        <div class="td-min">
                          <template v-for="(crouse,cindex) in item.SloverResultList">
                            <div v-cloak class="attend-cont-tit" :key="cindex" :style="{'background': crouse.bg,'borderLeftColor':crouse.color,'color':crouse.color}" v-if="crouse.Day == week.val  && crouse.AMorPM == sectionTd.AMorPM && crouse.AMorPMOrder == sectionTd.AMorPMOrder">
                              <p class="nowrap" :title="crouse.SubjectName"><b>{{crouse.SubjectName}}</b></p>
                              <p v-if="isShwowTeacher" class="nowrap" :title="crouse.TeacherName">{{crouse.TeacherName}}</p>
                              <p v-if="isShwowHouse" class="nowrap" :title="crouse.ClassName">{{crouse.ClassName}}</p>
                              <p v-if="isShwowTime && crouse.StartTime != null && crouse.EndTime != null" class="nowrap" :title="crouse.StartTime.substring(0,5)+'-'+crouse.EndTime.substring(0,5)">
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        ...
        // 获取教室课表
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetSloverResultHouseList`, {
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetHousesTree`, {
          classTableIDs: this.classTableIDs
        })
        if (res.success) {
          this.xy.unloading()
      ...
      exportFile () {
        ...
        if (this.isHorizontal) {
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportSloverResultHouseList`, obj)
        } else {
          if (this.subExport === 'export') {
            // 导出分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExporPartHouseClassTimes`, obj)
          } else {
            // 打印分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExporPrintingHouseClassTimes`, obj)
          }
        }
