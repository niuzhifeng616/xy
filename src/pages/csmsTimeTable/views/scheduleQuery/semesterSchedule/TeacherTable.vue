        ...
        <div class="left-options">
          ...
          <div class="options-body">
            <div class="search-opt">
              <Input search placeholder="请输入用户名/工号/姓名" v-model.trim="leftSearchName" @on-change="changeTree" clearable />
            </div>
            ...
            <div v-for="(item,index) in subjectData" :key='index' style="margin-top:15px;">
              <div class="subject-box">
                <Checkbox :indeterminate="item.indeterminate" :value="item.checkAll" @click.prevent.native="handleCheckAll(item,index)" :disabled="leftSearchName?true:false" >{{item.DisciplineName}}</Checkbox>
              </div>
              <CheckboxGroup class="teacher-box" v-model="item.selectSubject" disabled @on-change="subjectChange(item,index)">
                <template v-for="(subject,cindex) in item.list">
                  <Checkbox class="teacher-lable" :label="subject.TeacherID" :key="cindex" v-if="subject.FullName.indexOf(leftSearchName) >= 0 || subject.UserName.indexOf(leftSearchName) >= 0|| subject.SchoolCardNumber.indexOf(leftSearchName) >= 0">
        ...
        <div v-if="datatable.length>0">
                ...
                <table id="right-table2" class="query-table horizontal-version">
                  <tbody>
                    <tr v-for="(item, index) in datatable" :key="index" ref="trListRight">
                      <template v-for="(sectionTd,index) in sectionData">
                        <td v-cloak :key="index" v-if="(sectionTd.Day!=5&&sectionTd.Day!=6)||isContainWeekend">
                          <div class="td-min">
                            <template v-if='isHaveOneSelected'>
                              <template v-for="(crouse,cindex) in item.SloverResultList">
                               <div v-cloak class="attend-cont-tit" :key="cindex" :style="{'background': crouse.bg,'borderLeftColor':crouse.color,'color':crouse.color}" v-if="crouse.Day == sectionTd.Day && crouse.AMorPM == sectionTd.AMorPM  && crouse.AMorPMOrder == sectionTd.AMorPMOrder">
          ...
          <div style="overflow:auto;max-height:600px;">
            <template v-for="(item,index) in datatable">
              <div class="single" v-if="!isHorizontal" :key="index">
                  ...
                  <thead>
                    <tr>
                      ...
                      <template v-for="(week,cindex) in weekData">
                        <th :key="cindex" v-if="(week.val!=5&&week.val!=6)||isContainWeekend" v-cloak>{{week.title}}</th>
                      </template>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(sectionTd,index) in festList" :key="index">
                      ...
                      <template v-for="(week,cindex) in weekData">
                        <td v-cloak :key="cindex" v-if="(week.val!=5&&week.val!=6)||isContainWeekend">
                          <div class="td-min">
                            <template v-if='isHaveOneSelected'>
                               <template v-for="(crouse,kindex) in item.SloverResultList">
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        ...
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetSloverResultSessionTimeTableTeacher`, {
          classTableIDs: this.classTableIDs,
          disciplineID: '',
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/GetSloverResultSessionTimeTableTeacherIncludeTeacher`, {
          classTableIDs: this.classTableIDs
        })
        if (res.success) {
          let resData = res.data
      ...
      exportFile () {
        ...
        if (this.isHorizontal) {
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportTotalSloverResultSessionTimeTableTeacher`, {
            classTableIDs: this.classTableIDs,
          ...
          if (this.subExport === 'export') {
            // 导出分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportPartSloverResultSessionTimeTableTeacher`, {
              classTableIDs: this.classTableIDs,
              disciplineID: disciplineID,
              teacherID: this.teacherID,
              isContainWeekend: this.isContainWeekend,
            ...
            // 打印分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportSessionTimeTable/ExportPartPrintSloverResultSessionTimeTableTeacher`, {
              classTableIDs: this.classTableIDs,
              disciplineID: disciplineID,
              teacherID: this.teacherID,
              isContainWeekend: this.isContainWeekend,
      ...
      printLoad () {
        ...
        let routeUrl = this.$router.resolve({
          path: '/xyHome/semesterTeacherPrint',
