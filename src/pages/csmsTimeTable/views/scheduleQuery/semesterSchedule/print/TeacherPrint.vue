...
<div class="print-body">
    <div class="fix-down" v-if="isShowDown" @click="downPDF">打印课表</div>
      <div v-for="item in datatable" :key="item.FullName"
            :style="{'height':isShwowCourse&&isShwowClass&&isShwowHouse?'1140px':'558px','marginBottom':'40px'}">
                      ...
                      <template v-for="week in weekData">
                          ...
                          <template v-for="crouse in item.SloverResultList">
                             ...
                             <!-- 课程 -->
                                ...
                                <!-- 班级 -->
                                <div v-if="isShwowClass&&crouse.CoursePlanTableFullName!=null">
                                    <p class="nowrap other-name" v-if="crouse.CoursePlanTableFullName.length>10">{{crouse.CoursePlanTableFullName.substr(0,9)}}..</p>
                                    <p class="nowrap other-name" v-if="crouse.CoursePlanTableFullName.length<11">{{crouse.CoursePlanTableFullName}}</p>
                                </div>
                                <!-- 教学楼 -->
...
<script>
  export default {
    ...
    created () {
      ...
      this.isShwowCourse = print.isShwowCourse
      this.isShwowClass = print.isShwowClass
