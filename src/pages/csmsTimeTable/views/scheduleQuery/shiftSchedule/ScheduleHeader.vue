    ...
    data () {
      return {
        ...
        btnData: [
          {
            name: '教学班学生名单',
            url: '/ShiftScheduleTotal'
          },
          {
            name: '行政班学生去向',
            url: '/ShiftScheduleAdministrative'
          }
        ],
        clickUrl: '/ShiftScheduleTotal'
      }
    },
    created () {
      ...
      this.$router.push({ path: `/xyhome/ShiftScheduleHeader${this.btnData[0].url}`, query: this.$store.state.common.menuInfo })
    },
    mounted: function () {
    },
    methods: {
        ...
        if (res.success) {
          ...
          if (resData.OpearationPermissionList.length > 0) {
            for (var i = 0; i < resData.OpearationPermissionList.length; i++) {
              if (resData.OpearationPermissionList[i].OperationPermissionCode === 'StudentList') {
                this.btnData.push({
                  name: '教学班学生名单',
                  url: '/ShiftScheduleTotal'
                })
              };
              if (resData.OpearationPermissionList[i].OperationPermissionCode === 'StudentWalk') {
                this.btnData.push({
                  name: '行政班学生去向',
                  url: '/ShiftScheduleAdministrative'
                })
      ...
      scheduleChange () {
        // this.isIndex = i
        this.$router.push({ path: `/xyhome/ShiftScheduleHeader${this.clickUrl}`, query: this.$store.state.common.menuInfo })
      },
      // 返回
      back () {
        this.$router.push({
          path: '/xyhome/ShiftScheduleConditions',
