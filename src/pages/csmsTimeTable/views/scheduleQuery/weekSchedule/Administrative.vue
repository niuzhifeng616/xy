  ...
  <div class="xy-content-module" ref="Schedule">
    ...
    <Row>
      <i-col span="4">
        <div class="left-options">
          ...
          <div class="options-body">
            <Checkbox label="0" v-model="allTeacher" class="schedule-subject administrative-subject" @on-change="changeAllTeacher" v-show='subjectData.length>0'>全部班级</Checkbox>
            <div v-for="(item,index) in subjectData" :key='index' style="margin-top:15px;">
              <div class="subject-box">
                <Checkbox :indeterminate="item.indeterminate" :value="item.checkAll" @click.prevent.native="handleCheckAll(item,index)">{{item.StudentGradeForStudentYearName}}</Checkbox>
              </div>
              <CheckboxGroup class="teacher-box" v-model="item.selectSubject" @on-change="subjectChange(item,index)">
                <template v-for="(subject) in item.Orgs">
                  <Checkbox class="teacher-lable" :label="subject.StudentOrganizationID" :key="subject.StudentOrganizationID">
      ...
      <i-col span="20" class="right-table" style=" border-left: 1px solid #E8E8E8;">
        <div style="overflow:auto;max-height:600px;" v-if="datatable.length>0">
            ...
            <table class="query-table">
              ...
              <tbody>
                <tr v-for="(sectionTd,sindex) in festList" :key="sindex">
                  ...
                  <template v-for="(week,weindex) in weekData">
                    <td v-cloak :key="weindex" v-if="(week.val!=5&&week.val!=6)||isContainWeekend">
                      <div class="td-min">
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        ...
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetBaseWeekTimeTableAdministrativeClass`, {
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetBaseWeekTimeTableAdministrativeClassClass`, {
          academicSessionID: this.session.AcademicSessionID,
          schoolDistrictID: this.school.SchoolDistrictID,
          studentGradeID: this.grade.StudentGradeID,
          startDate: this.startDate,
      ...
      exportFile () {
        ...
        //   this.studentGradeForStudentYearID = this.selectSubject.StudentGradeForStudentYearID
        //   this.studentOrganizationID = this.selectSubject.StudentOrganizationID
        // } else {
        ...
        if (this.subExport === 'export') {
          // 导出版
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportBaseWeekTimeTableAdministrativeClass`, obj)
        } else {
          // 打印版
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportPrintSloverResultAdministrativeClassTimes`, obj)
        }
      ...
      printLoad () {
        ...
        let routeUrl = this.$router.resolve({
          path: '/xyHome/WeekPrintAdministrative'
        })
