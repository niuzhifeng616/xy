<template>
  <div class="xy-content-module">
    ...
    <Button type="primary" class="export-btn xy-primary" @click="modalExportFun">导出</Button>
            ...
            <table class="query-table">
                ...
                <tbody>
                    <tr v-for="(sectionTd,sindex) in festList" :key="sindex">
                        ...
                        <td v-cloak v-for="(week,windex) in weekData" :key="windex">
                            <div class="td-min">
                                <template v-for="(crouse,cindex) in item.SloverResultList">
                                    <div v-cloak class="attend-cont-tit"
                                        :key="cindex"
                                        :class="crouse.IsAdjustment ? 'dis-td-bg': ''"
                                        :style="{'background':  crouse.IsAdjustment ? '#ccc': crouse.bg,'borderLeftColor':crouse.color,'color':crouse.color}"
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetSloverResultClassTimeList`, {
          schoolDistinctID: this.school.SchoolDistrictID,
          studentGradeID: this.grade.StudentGradeID,
          //   schoolDistinctID: 20305,
          //   studentGradeID: 1,
          startDate: this.startDate,
          endDate: this.endDate,
      ...
      exportFile () {
        this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportSloverResultClassTime?schoolDistinctID=
        ${this.school.SchoolDistrictID}
        &studentGradeID=${this.grade.StudentGradeID}&startDate=${this.startDate}
        &endDate=${this.endDate}&ContainWeekEnd=${this.isContainWeekend}
