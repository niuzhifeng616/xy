<template>
  <div class="xy-content-module" ref="Schedule">
    ...
    <Row>
            ...
            <div v-for="(item,index) in subjectData" :key='index' style="margin-top:15px;">
              ...
              <CheckboxGroup class="teacher-box"  v-model="item.selectSubject" @on-change="subjectChange(item,index)">
                <template v-for="(subject) in item.Orgs">
                  <Checkbox class="teacher-lable" :label="subject.StudentOrganizationID" :key="subject.StudentOrganizationID+item.StudentGradeForStudentYearID"  v-if="subject.StudentOrganizationName.indexOf(leftSearchName) >= 0">
      ...
      <i-col span="20" class="right-table" style="border-left: 1px solid rgb(232, 232, 232);">
        <div v-if="datatable.length>0">
            ...
            <div id="left-div">
              ...
              <div id="left-div2">
                <table id="left-table2" class="query-table horizontal-version">
                  <tbody>
                    <tr v-for="item in datatable" :key="(item,index)" ref="trListLeft">
                      <td>
                    ...
                    <tr v-for="(item,index) in datatable" :key="index" ref="trListRight">
                      <template v-for="(sectionTd,sindex) in sectionData">
                        <td v-cloak :key="sindex" v-if="(sectionTd.Day!=5&&sectionTd.Day!=6)||isContainWeekend">
                          <div class="td-min">
                            <template v-for="(crouse,cindex) in item.ElectiveList">
          ...
          <!-- 分表 -->
          <div style="overflow:auto;height:600px;" v-if="!isHorizontal">
    ...
    <Modal v-model="modalExport" :transfer="false" :closable="false">
      ...
      <p slot="header" v-else>导出学生课表分班</p>
  ...
  export default {
    ...
    methods: {
      ...
      async getSloverResultOverallTimes () {
        ...
        // this.datatable = []
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetElectiveForClassTableList`, {
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetAllStudentOrg`, {
          schoolDistinctID: this.school.SchoolDistrictID,
          studentGradeID: this.grade.StudentGradeID,
          startDate: this.startDate,
          endDate: this.endDate
      ...
      exportFile () {
        ...
        if (this.isHorizontal) {
          this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportTotalStudentClassTimes`, {
            schoolDistinctID: this.school.SchoolDistrictID,
            studentGradeID: this.grade.StudentGradeID,
            searchName: this.searchName,
            startDate: this.startDate,
          ...
          if (this.subExport === 'export') {
            // 导出分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExporPartStudentClassTimes`, {
              schoolDistinctID: this.school.SchoolDistrictID,
              studentGradeID: this.grade.StudentGradeID,
              searchName: this.searchName,
              startDate: this.startDate,
            ...
            // 打印分表
            this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExporPartStudentClassTimesZip`, {
              schoolDistinctID: this.school.SchoolDistrictID,
              studentGradeID: this.grade.StudentGradeID,
              searchName: this.searchName,
              startDate: this.startDate,
