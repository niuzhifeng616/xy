<template>
  <div class="xy-content-module" ref="Schedule">
    ...
    <Button class="export-btn xy-primary" @click="modalExportFun">导出</Button>
    <Row>
      <i-col span="4">
        <div class="left-options">
          ...
          <div class="options-body">
            ...
            <CheckboxGroup class="teacher-box" v-model="selectSubject" @on-change="subjectChange()">
              <template v-for="(subject) in subjectData">
                <Checkbox class="teacher-lable" :label="subject.DisciplineID" :key="subject.DisciplineID">
      ...
      <i-col span="20" class="right-table" style="border-left: 1px solid #E8E8E8;">
        ...
        <div v-if="datatable.length>0">
                ...
                <table id="right-table2" class="query-table horizontal-version">
                  <tbody>
                    <tr v-for="(item,index) in datatable" :key="index" ref="trListRight">
                      <td v-cloak v-for="(sectionTd,kindex) in sectionData" :key="kindex">
                        <div class="td-min">
          ...
          <div style="overflow:auto;max-height:600px;">
            <table class="query-table" v-if="!isHorizontal">
              <thead>
                <tr>
                  <th>
              ...
              <tbody>
                <tr v-for="(section,index) in sectionData" :key="index">
                  <td v-if="section.isNextDay" :rowspan="sectionLen" v-cloak>
                    {{weekData[section.Day].title}}
                  </td>
                  <td v-cloak>
                    {{sectionData[index].AMorPMOrderName}}
                  </td>
                  <td v-cloak v-for="(item,cindex) in datatable" :key="cindex">
                    <div class="td-min">
                      <template v-for="(crouse,kindex) in item.SloverResultList">
                        <div v-cloak class="attend-cont-tit" :key="kindex" :class="crouse.IsAdjustment ? 'dis-td-bg': ''" :style="{'background': crouse.IsAdjustment ? '#ccc': crouse.bg,'borderLeftColor':crouse.color,'color':crouse.color}" v-if="crouse.Day == section.Day && crouse.AMorPM == section.AMorPM  && crouse.AMorPMOrder == section.AMorPMOrder">
                          <p class="nowrap" :title="crouse.SubjectName"><b>{{crouse.SubjectName}}</b></p>
                          <p v-if="isShwowTeacher" class="nowrap" :title="crouse.TeacherName">{{crouse.TeacherName}}</p>
                          <p v-if="isShwowHouse" class="nowrap" :title="crouse.HouseName">{{crouse.HouseName}}</p>
                          <p v-if="isShwowTime && crouse.StartTime != null && crouse.EndTime != null" class="nowrap" :title="crouse.StartTime.substring(0,5)+'-'+crouse.EndTime.substring(0,5)">
  ...
  export default {
    ...
    watch: {
      aa: {
        handler (val) {
          ...
          let dom = document.querySelector('#right-div2')
          if (dom !== null) {
            dom.scrollTop = 0
            dom.scrollLeft = 0
          }
    ...
    methods: {
      ...
      async getSloverResultOverallTimes (ids) {
        ...
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetBaseWeekTimeTableOverall`, {
      ...
      async getSloverResultOverallTimesDiscipline () {
        this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/GetBaseWeekTimeTableOverallDiscipline`, {
          academicSessionID: this.session.AcademicSessionID,
          schoolDistrictID: this.school.SchoolDistrictID,
          studentGradeID: this.grade.StudentGradeID,
          startDate: this.startDate,
      ...
      exportFile () {
        ...
        // 导出
        this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportBaseWeekTimeTableOverall?`)
        this.xy.downFile(`${this.$store.state.apiPath}/api/ReportWeekTimeTable/ExportBaseWeekTimeTableOverall`, {
          academicSessionID: this.session.AcademicSessionID,
          schoolDistrictID: this.school.SchoolDistrictID,
          studentGradeID: this.grade.StudentGradeID,
          startDate: this.startDate,
          endDate: this.endDate,
          disciplineID: this.selectSubject.length > 0 ? this.selectSubject.join(',') : this.disciplineIDs,
