  ...
  <div class="clearfix" style="height: 100%;">
    <div class="left-data fl">
      <div class="import-btn"
            :class="isOperation !== 0? 'disabled':''"
            @click="ImportClick"><i class="iconfont">&#xe6c0;</i>导入权限</div>
      <div v-for="(item, index) in crouseTree" :key="index">
        <div class="left-item-title">{{item.title}}</div>
        <radio-group v-model="curSubject"
                      vertical
                      @on-change="radioChange"
                      style="max-width:240px">
          <radio :label="subject.id"
                  v-for="(subject, n) in item.children"
                  :key="n"
                  class="nowrap">
            <span class="f14 radio-name"
                  :title="subject.title">{{subject.title}}</span>
          </radio>
        </radio-group>
      </div>
    </div>
      ...
      <div class="clearfix mb-15" style="margin-top:10px" v-if="curSubject!==-1">
        <div class="fl">
          ...
          <Button class="xy-danger"
                  @click="delBatch"
                  :disabled="isOperation !== 0||datatable.length===0">批量删除</Button>
          <Button class="xy-danger"
                  @click="delAll"
                  :disabled="isOperation  !== 0||datatable.length===0">全部删除</Button>
        </div>
        <div class="fr pr">
          <Input search
  ...
  export default {
    ...
    methods: {
      ...
      async getTP (aid, type) {
        ...
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/GetCourseplanPermissionList`, {
      ...
      delOne (row) {
        ...
        _this.$Modal.confirm({
          ...
          async onOk () {
            let res = await _this.xy.post(`${_this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/CourseplanPermissionDelete`, {
      ...
      // 批量删除
      delBatch () {
        if (!this.isNullData && this.isOperation === 0) {
          if (this.selectionAll.length === 0) {
            ...
            _this.$Modal.confirm({
              ...
              async onOk () {
                let res = await _this.xy.post(`${_this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/CourseplanPermissionDeleteByIds`, {
                  ids: ids,
                  ClassTableID: _this.classTableID
                })
                if (res.success) {
      ...
      delAll () {
        ...
        if (!_this.isNullData && _this.isOperation === 0) {
          _this.$Modal.confirm({
            ...
            content: '您确定要删除所有学生吗？',
            async onOk () {
              let res = await _this.xy.post(`${_this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/CourseplanPermissionDeleteAll?classTableID=${_this.classTableID}
      ...
      ImportClick () {
        if (this.isOperation === 0) {
          ...
          this.upload = {
            ...
            size: 10240,
            upParams: {},
            upUrl: `${this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/Import?classTableID=${this.classTableID}`, // 上传文件url
            templateUrl: `${this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/DownloadTemplateImport`, // 模板下载url
            errorUrl: `${this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/DownloadExceptionImport` // 下载错误文件url
          }
        }
      },
      modalSubmit () {
      ...
      async addComplate () {
        ...
        this.xy.loading()
        let res = await this.xy.post(`${this.$store.state.apiPath}/api/SelectSchoolBasedRuleSetting/CreateCourseplanPermission`, {
          ClassTableID: this.classTableID,
...
.leftTreeTitle .editorTree {
  background-color: #43bbf7;
  padding: 2px 5px;
  border-radius: 5px;
  cursor: pointer;
}
.leftTreeTitle .editorTree i {
  color: #fff;
}
.xy-tree {
  border: 1px solid #e3e3e3;
...
.treeModel {
  ...
  z-index: 10;
  background: #fff;
  box-shadow: 1px 2px 15px 3px #e3e3e3;
  overflow: scroll;
}
.treeModel .ivu-tree {
  padding: 15px 20px;
...
.modal-cradeList label {
  margin: 0 5px;
}
.modal-cradeList .ivu-date-picker {
  width: 120px;
}
.modal-cradeList .ivu-input-type {
  width: 120px;
}
/*创建学习阶段*/
/*.creatGradeList .grade-check .ivu-checkbox-wrapper{
    padding:10px 0;
}*/
.creatGradeList .grade-check .ivu-checkbox-wrapper label {
  margin: 0 6px;
}
/*创建学届*/
.creatYearListModal ul li {
  font-size: 14px;
  padding: 5px 0;
}
/*checkbox样式*/
