...
<script>
  export default {
    ...
    mounted () {
      ...
      let arrBar1 = []
      if (this.grade === 7) {
        arrBar1 = arr.slice(indexBar1 + 1, indexBar2)
      } else {
        arrBar1 = arr.slice(indexBar1 + 1)
      }
      ...
      if (this.grade === 7) {
        ...
        this.seriessubjectValueData.push([data5[0]], [data5[1]], [data5[2]], [data5[3]])
      }
      ...
      // console.log(data3.concat(data4), '??????312312312')
      ...
      console.log(this.subjectEvaluation)
      this.chart2()
      this.setbarEchart()
    },
    methods: {
      ...
      setbarEchart (myChart, option) {
        ...
        that.$nextTick(function () {
          const barEchart = this.echarts.init(this.$refs.barEchart)
          option = {
            title: {
              text: '学科终结性评价',
              left: '40%',
            ...
            legend: {
              data: [`${this.valueData1L}`, '班级均分', '班级最高分', '年级均分', '年级最高分'],
              top: '94%',
              itemWidth: 14,
              itemHeight: 14,
              textStyle: {
            ...
            series: [
              // {
              //   name: `${this.dataAll[0]['姓名']}`,
              //   type: 'bar',
              //   data: this.seriessubjectValueData[0],
              //   barWidth: 40
              // },
              {
                name: '班级均分',
              ...
              },
              {
                name: '班级最高分',
                type: 'bar',
                data: this.seriessubjectValueData[1],
                barWidth: 40
              },
              {
                name: '年级均分',
                type: 'bar',
                data: this.seriessubjectValueData[2],
                barWidth: 40
              },
              {
                name: '年级最高分',
                type: 'bar',
                data: this.seriessubjectValueData[3],
                barWidth: 40
              }
            ]
          }
          ...
          setTimeout((async) => {
            var imgSrc3 = barEchart.getDataURL()
            // 渲染到图表上面，遮住图表
