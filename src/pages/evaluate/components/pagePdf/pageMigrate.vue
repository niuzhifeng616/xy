<template>
  <div class='pageFamily'>
    ...
    <div class="page-num">
      <div class="count">{{pageObj['.pagePhecda']+5}}</div>
    </div>
  </div>
</template>
<script>
  export default {
    props: ['migrateRes', 'pageObj'],
    name: '',
    data () {
      return {
        arrRadar2: [],
    ...
    created () {},
    mounted () {
      this.level = this.migrateRes[0]['综评等级']

      let arr = Object.keys(this.migrateRes[0])
      let index2 = 0
      let index3 = 0
...
.pageFamily{
  ...
  .title{
    width: 281px;
    height: 60px;
    background: url('./../../views/qualityReport/img/phecda/migrateTitle.png') no-repeat;
    background-size: 100% 100%;
    margin: auto;
  }
  .content{
    ...
    height: 596px;
    margin: 50px 70px 0;
    background: url('./../../views/qualityReport/img/phecda/migrateContent.png') no-repeat;
    background-size: 100% 100%;
  }
  .data-box{
