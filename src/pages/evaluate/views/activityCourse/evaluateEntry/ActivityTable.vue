  ...
  <div class="xy-content-module">
    ...
    <div class="xy-content-body">
      <XyAntTable
        ...
        :scrollx="'max-content'"
        >
         <div slot="ActivityCourseProjectName" style="min-width:150px" slot-scope="text,record">
            <span class="table-el-hover" @click="evaluateEntry(record)">
              {{record.ActivityCourseProjectName}}
            </span>
          </div>
          <span slot="operation" slot-scope="text, record">
            <a @click="evaluateEntry(record)" >评价录入</a>
          </span>
      </XyAntTable>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      evaluateEntry (row) {
        ...
        let start = new Date(row.ActivityCourseEvaStartTime)
        let end = new Date(row.ActivityCourseEvaEndTime)
        let isEdit = true
        if (date < start) {
          this.xy.msgW('评价时间未开始，无法进行评价录入。')
          return
        } else if (date > end) {
          isEdit = false
        }
        this.$store.commit('common/getThreeMenuName', row.ActivityCourseProjectName)
        this.$store.commit('common/getParam', { id: row.ActivityCourseProjectID, type: row.ActivityCourseScoreType, isEdit: isEdit })
        this.$router.push({
          path: '/xyhome/evaEntry',
      ...
      async loadTable () {
        ...
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/ActivityCourseProject/GetTeacherProjectByPage`, obj)
        if (res.success) {
