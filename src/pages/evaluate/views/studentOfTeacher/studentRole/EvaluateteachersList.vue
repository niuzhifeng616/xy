...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goDetail (row) {
        this.$store.commit('evaluate/changeEvaluateDetails', row)
        this.$store.commit('common/getParam', { pro: 'EvaluateteachersList' })
        this.$store.commit('common/getThreeMenuName', row.TeachingProjectName)
        this.$router.push({
          path: '/xyHome/EvaluateDetails',
      ...
      async getSchool () {
        // this.xy.loading()
        let res = await this.xy.get(`${this.$store.state.apiPath}/api/StudentTeachingReport/GetSchoolDistrictByRole`, { 'pageType': 1 })
        if (res.success) {
          ...
          if (res.success) {
            ...
            if (index !== -1) {
              ...
              if (index1 !== -1) {
                ...
                that.yearSession = [0, 0] // 产品要求默认展示所有学年/所有学期
              } else {
