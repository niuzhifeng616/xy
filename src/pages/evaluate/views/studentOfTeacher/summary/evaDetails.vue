<template>
  <div class="xy-content-module">
      <div class="xy-content-title summary-footer">
        <div>
          <div v-if="params.row.TeacherID" style="display:flex;align-items:center;margin-left:10px;padding-top:6px;">
              <img
                v-if="params.row.Icon !==''"
                class="notice-ajust-img"
                :src="`${params.row.Icon}`"
                :alt="params.row.TeacherFullName"
                >
              <div
                v-else
                class="notice-ajust-person"
                :class="params.row.TeacherSex === 0 ? 'woman' : ''"
                >
                {{ showName(params.row.TeacherFullName) }}
              </div>
              <div class="full-box">
                  <div>{{params.row.TeacherFullName}}</div>
                  <div>({{params.row.TeacherName}})</div>
              </div>
          </div>
        </div>
        <div v-if="params.row.TeacherID" class="footer-bigBox">
            <div class="left-box">
              <div class="titSmall-box">
                  {{params.quesOrder}}.&nbsp;&nbsp;{{params.title}}
              </div>
                <span v-if="total>0">采集到<span>{{total}}</span>份</span>
            </div>
            <div class="right-box">
      ...
      <div v-if="total===0" class="no-data-box">
      ...
      <div v-else class="xy-content-body details-box-wrap">
        ...
        <Page
          v-if="detailData.data.length>0"
          class="xy-page text-right pr"
