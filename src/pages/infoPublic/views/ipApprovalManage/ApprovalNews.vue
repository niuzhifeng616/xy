  ...
  <div class="details-wrap">
    ...
    <div class="approval-cont">
      <Form label-position="top" class="details-info">
        ...
        <FormItem label="">
          <Table v-bind:columns="columns"
                v-bind:data="datatable"></Table>
        </FormItem>
    ...
    <Modal
      ...
      width="900"
      v-bind:styles="{top: '20px'}"
      title="查看新闻"
      v-bind:mask-closable="false"
      v-bind:closable="true"
      @on-ok="noticeOk"
      ok-text="确定"
      @on-cancel="noticeCancel">
      <div class="dialog-model-content">
        <Form>
          <FormItem label="新闻类型" prop="noticeType">
            {{seeNotice.NewsTypeName}}
          </FormItem>
          <FormItem label="新闻名称" prop="noticeName">
            {{seeNotice.NewsTitle}}
          </FormItem>
          <FormItem label="新闻图片" prop="noticeName">
             <img v-bind:src="seeNotice.NewsImageLink" style="width:300px;"/>
          </FormItem>
          <FormItem label="新闻内容" prop="noticeCont" style="overflow:auto;" v-cloak>
            <div v-html="seeNotice.NewsContent">
              {{seeNotice.NewsContent}}
            </div>
          </FormItem>
          <FormItem label="发布时间" prop="noticeCont">
            {{xy.moment(seeNotice.StartDate).format('YYYY/MM/DD HH:mm')}} - {{xy.moment(seeNotice.EndDate).format('YYYY/MM/DD HH:mm')}}
          </FormItem>
        </Form>
      </div>
    </Modal>
    ...
    methods: {
      ...
      async GetPublicApproveById () {
        ...
        if (rs.success) {
          ...
          this.datatable = rs.data.NewsList
        }
      },
