<template>
  <div class="details-wrap">
    ...
    <div class="approval-cont">
      ...
      <!-- 查看通知 -->
      <Modal v-model="noticeCreate"
             v-bind:transfer="false"
             v-bind:styles="{top: '20px'}"
             title="查看通知"
             v-bind:mask-closable="false"
             v-bind:closable="true"
             @on-ok="noticeCreate = false"
             ok-text="确定"
             @on-cancel="noticeCreate = false">
        <div class="dialog-model-content">
          <Form>
            <FormItem label="通知类型" prop="noticeType">
              {{seeNotice.InformTypeName}}
            </FormItem>
            <FormItem label="通知名称" prop="noticeName">
              {{seeNotice.InformName}}
            </FormItem>

            <FormItem label="字体颜色" prop="noticeColor">
              {{seeNotice.InformColorName}}
            </FormItem>
            <FormItem label="发布内容">
                        <span style="display: block; line-height: 2.9; word-wrap:break-word;">
                            {{seeNotice.InformContent}}
                        </span>
            </FormItem>
            <FormItem label="发布时间" prop="noticeCont" v-cloak>
              {{xy.moment(seeNotice.StartDate).format('YYYY-MM-DD HH:mm')}} -
              {{xy.moment(seeNotice.EndDate).format('YYYY-MM-DD HH:mm')}}
            </FormItem>
          </Form>
        </div>
      </Modal>
    ...
    methods: {
      ...
      async GetPublicApproveById () {
        ...
        if (rs.success) {
          ...
          this.datatable = rs.data.InfoList
        }
      },
