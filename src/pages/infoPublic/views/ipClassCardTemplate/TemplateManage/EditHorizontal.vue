    ...
    <div class="xy-content-body pr">
        <div class="clearfix">
            <div class="fl">
                ...
                <Input v-model="TemplateName" v-cloak style="width:200px;margin-right:10px;" maxlength="10" :disabled="isDisabled" />
                <span style="padding-top: 4px;">模板简介：</span>
                <Input v-model="TemplateDescribe" v-cloak style="width:300px;" maxlength="30" :disabled="isDisabled" />
                <tooltip placement="top" v-show="!isDisabled">
            ...
            <div class="fr">
                <i-button type="primary" @click="addTemplate" v-if="!isDisabled">保存</i-button>
            </div>
        </div>
        <div class="tamplet-box-container">
            ...
            <div class="tamplate-box-right" v-if="!isDisabled">
                    ...
                    <div v-for="(item,index) in dragList" :key="index" v-cloak>
                        ...
                        <div class="dragebox-template">
                            <div v-for="(drags,index) in item.model"
                                 ...
                                 @dragend="dragend">
                                <div class="pr">
                                    <img :style="{border: skinN === 'ksl' ? '1px solid #e9e9e9' : '',borderRadius: skinN === 'ksl' ? '8px' : ''}" :src="require('@/assets/common/infoPublic/classCard/'+drags.ControlImg)" :class="'dragbox-template-img-size-'+drags.ControlImgStyle" class="cursor-grab" />
  ...
  export default {
          ...
          $('#templateModule').on('click', '.del-template-img', function () {
            ...
            for (let i = 0; i < that.addTemplateList.length; i++) {
              if (Number($(this).attr('data-indexs')) === -1) {
                ...
                let a = Number($(this).attr('data-indexs'))
                let b = Number(that.addTemplateList[i].index)
                if (a === b) {
    ...
    methods: {
      ...
      async getTemplateList () {
        ...
        if (res.success) {
          ...
          if (this.TemplateLayout !== null && this.TemplateLayout.length > 0) {
            // let editTemplateArr = [];
            let renderData = []
            for (let e = 0; e < this.TemplateLayout.length; e++) {
              for (let w = 0; w < this.dragList.length; w++) {
                for (let c = 0; c < this.dragList[w].model.length; c++) {
            ...
            for (let q = 0; q < renderData.length; q++) {
              if (renderData[q].controlImgStyle === 1) {
                if ($('.xy-drag-box').eq(renderData[q].index).attr('data-disable') !== 1) {
                  let nowHtml = document.getElementsByClassName('xy-drag-box')[renderData[q].index]
                  $('.xy-drag-box').eq(renderData[q].index).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-style', 1)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-one', renderData[q].index)
                  let tHtml = ''
                  if (!this.isDisabled) {
                    tHtml = `<div style="position:absolute;
                        ...
                        data-nowi="${renderData[q].index}">
                          <img src="${require('@/assets/common/infoPublic/classCard/' + renderData[q].ControlImg)}" style='width:100%;height:100%;'>
                          <i class="iconfont del-template-img" data-indexs="${renderData[q].index}">&#xe6d8;</i>
                        </div>`
                  } else {
                    tHtml = `<div style="position:absolute;
                    left:${nowHtml.offsetLeft}px;
              ...
              if (renderData[q].controlImgStyle === 2) {
                if ($('.xy-drag-box').eq(renderData[q].index).attr('data-disable') !== 1) {
                  let nowHtml2 = document.getElementsByClassName('xy-drag-box')[renderData[q].index]
                  $('.xy-drag-box').eq(renderData[q].index).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-style', 2)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-one', renderData[q].index)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-two', renderData[q].index + 4)
                  $('.xy-drag-box').eq(renderData[q].index + 4).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index + 4).attr('data-disable', '1')
                  let tHtml2 = ''
                  if (!this.isDisabled) {
                    tHtml2 = `<div style="position:absolute;
                        left:${nowHtml2.offsetLeft}px;
                        top:${nowHtml2.offsetTop}px;
                        width:${nowHtml2.offsetWidth}px;
                        height:${(nowHtml2.offsetHeight * 2 + 20)}px;
                        background:rgba(0,0,0,0.5);
                        border-radius:6px;"
                        class="hover-template"
                        data-nowi="${renderData[q].index}">
              ...
              if (renderData[q].controlImgStyle === 3) {
                if ($('.xy-drag-box').eq(renderData[q].index).attr('data-disable') !== 1) {
                  let nowHtml3 = document.getElementsByClassName('xy-drag-box')[renderData[q].index]
                  $('.xy-drag-box').eq(renderData[q].index).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-style', 3)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-one', renderData[q].index)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-two', renderData[q].index + 1)
                  $('.xy-drag-box').eq(renderData[q].index + 1).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index + 1).attr('data-disable', '1')
                  let tHtml3 = ''
                  if (!this.isDisabled) {
                    tHtml3 = `<div style="position:absolute;
                        left:${nowHtml3.offsetLeft}px;
                        top:${nowHtml3.offsetTop}px;
                        width:${(nowHtml3.offsetWidth * 2 + 20)}px;
              ...
              if (renderData[q].controlImgStyle === 4) {
                if ($('.xy-drag-box').eq(renderData[q].index).attr('data-disable') !== 1) {
                  let nowHtml4 = document.getElementsByClassName('xy-drag-box')[renderData[q].index]
                  $('.xy-drag-box').eq(renderData[q].index).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-style', 4)
                  $('.xy-drag-box').eq(renderData[q].index).attr('data-xy-one', renderData[q].index)
                  $('.xy-drag-box').eq(renderData[q].index + 1).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index + 1).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index + 4).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index + 4).attr('data-disable', '1')
                  $('.xy-drag-box').eq(renderData[q].index + 5).addClass('grid-lay-none')
                  $('.xy-drag-box').eq(renderData[q].index + 5).attr('data-disable', '1')
                  let tHtml4 = ''
                  if (!this.isDisabled) {
                    tHtml4 = `<div style="position:absolute;
                        left:${nowHtml4.offsetLeft}px;
                        top:${nowHtml4.offsetTop}px;
                        width:${(nowHtml4.offsetWidth * 2 + 20)}px;
              ...
              if (renderData[q].controlImgStyle === 5) {
                let nowHtml5 = document.getElementsByClassName('xy-drag-box')[0]
                $('.xy-drag-box').addClass('grid-lay-none')
                $('.xy-drag-box').attr('data-disable', '1')
                $('.xy-drag-box').attr('data-xy-style', 5)
                $('.xy-drag-box').attr('data-xy-one', 0)
                $('.xy-drag-box').attr('data-xy-two', 11)
                let tHtml = ''
                if (!this.isDisabled) {
                  tHtml = `<div style="position:absolute;
                      left:${nowHtml5.offsetLeft}px;
                      top:${nowHtml5.offsetTop}px;
                      width:${(nowHtml5.offsetWidth * 4 + 60)}px;
                      height:${(nowHtml5.offsetHeight * 3 + 40)}px;
                      background:rgba(0,0,0,0.5);
                      border-radius:6px;"
                      class="hover-template"
                      data-nowi="11">
            ...
            // for (let q = 0; q < this.TemplateLayout.length; q++) {
            //   for (let x = 0; x < editTemplateArr.length; x++) {
            ...
            //         if ($('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable') !== 1) {
            //           let nowHtml = document.getElementsByClassName('xy-drag-box')[this.TemplateLayout[q].index]
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-style', 1)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-one', this.TemplateLayout[q].index)
            //           let tHtml = ''
            //           if (!this.isDisabled) {
            //             // tHtml = "<div data-nowi='" + this.TemplateLayout[q].index + "' style='position:absolute;left:" + nowHtml.offsetLeft + 'px;top:' + nowHtml.offsetTop + 'px;width:' + nowHtml.offsetWidth + 'px;height:' + nowHtml.offsetHeight + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //             //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'>" +
            //             //   "<i class='fa fa-close del-template-img' data-indexs='" + this.TemplateLayout[q].index + "'></i></div>"
            //             tHtml = `<div style="position:absolute;
            //             left:${nowHtml.offsetLeft}px;
            //             top:${nowHtml.offsetTop}px;
            //             width:${nowHtml.offsetWidth}px;
            ...
            //               <img src="${require('@/assets/common/infoPublic/classCard/' + editTemplateArr[x].ControlImg)}" style='width:100%;height:100%;'>
            //               <i class="iconfont del-template-img" data-indexs="${this.TemplateLayout[q].index}">&#xe6d8;</i>
            //             </div>`
            //           } else {
            //             // tHtml = "<div data-nowi='" + this.TemplateLayout[q].index + "' style='position:absolute;left:" + nowHtml.offsetLeft + 'px;top:' + nowHtml.offsetTop + 'px;width:' + nowHtml.offsetWidth + 'px;height:' + nowHtml.offsetHeight + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //             //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'></div>"
            //             tHtml = `<div style="position:absolute;
            //             left:${nowHtml.offsetLeft}px;
            //             top:${nowHtml.offsetTop}px;
            //             width:${nowHtml.offsetWidth}px;
            ...
            //         if ($('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable') !== 1) {
            //           let nowHtml2 = document.getElementsByClassName('xy-drag-box')[this.TemplateLayout[q].index]
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-style', 2)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-one', this.TemplateLayout[q].index)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-two', this.TemplateLayout[q].index + 4)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 4).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 4).attr('data-disable', '1')
            //           let tHtml2 = ''
            //           if (!this.isDisabled) {
            //             // tHtml2 = "<div data-nowi='" + this.TemplateLayout[q].index + "' style='position:absolute;left:" + nowHtml2.offsetLeft + 'px;top:' + nowHtml2.offsetTop + 'px;width:' + nowHtml2.offsetWidth + 'px;height:' + (nowHtml2.offsetHeight * 2 + 20) + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //             //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'>" +
            //             //   "<i class='fa fa-close del-template-img' data-indexs='" + this.TemplateLayout[q].index + "'></i></div>"
            //             tHtml2 = `<div style="position:absolute;
            //             left:${nowHtml2.offsetLeft}px;
            ...
            //         if ($('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable') !== 1) {
            //           let nowHtml3 = document.getElementsByClassName('xy-drag-box')[this.TemplateLayout[q].index]
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-style', 3)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-one', this.TemplateLayout[q].index)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-two', this.TemplateLayout[q].index + 1)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 1).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 1).attr('data-disable', '1')
            //           let tHtml3 = ''
            //           if (!this.isDisabled) {
            //             // tHtml3 = "<div data-nowi='" + this.TemplateLayout[q].index + "' style='position:absolute;left:" + nowHtml3.offsetLeft + 'px;top:' + nowHtml3.offsetTop + 'px;width:' + (nowHtml3.offsetWidth * 2 + 20) + 'px;height:' + nowHtml3.offsetHeight + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //             //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'>" +
            //             //   "<i class='fa fa-close del-template-img' data-indexs='" + this.TemplateLayout[q].index + "'></i></div>"
            //             tHtml3 = `<div style="position:absolute;
            //             left:${nowHtml3.offsetLeft}px;
            ...
            //         if ($('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable') !== 1) {
            //           let nowHtml4 = document.getElementsByClassName('xy-drag-box')[this.TemplateLayout[q].index]
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-style', 4)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index).attr('data-xy-one', this.TemplateLayout[q].index)
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 1).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 1).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 4).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 4).attr('data-disable', '1')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 5).addClass('grid-lay-none')
            //           $('.xy-drag-box').eq(this.TemplateLayout[q].index + 5).attr('data-disable', '1')
            //           let tHtml4 = ''
            //           if (!this.isDisabled) {
            //             // tHtml4 = "<div data-nowi='" + this.TemplateLayout[q].index + "' style='position:absolute;left:" + nowHtml4.offsetLeft + 'px;top:' + nowHtml4.offsetTop + 'px;width:' + (nowHtml4.offsetWidth * 2 + 20) + 'px;height:' + (nowHtml4.offsetHeight * 2 + 20) + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //             //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'>" +
            //             //   "<i class='fa fa-close del-template-img' data-indexs='" + this.TemplateLayout[q].index + "'></i></div>"
            //             tHtml4 = `<div style="position:absolute;
            //             left:${nowHtml4.offsetLeft}px;
            ...
            //         let tHtml = ''
            //         if (!this.isDisabled) {
            //           // tHtml = "<div data-nowi='" + 11 + "' style='position:absolute;left:" + nowHtml5.offsetLeft + 'px;top:' + nowHtml5.offsetTop + 'px;width:' + (nowHtml5.offsetWidth * 4 + 60) + 'px;height:' + (nowHtml5.offsetHeight * 3 + 40) + "px;background:rgba(0,0,0,0.5);border-radius:6px;' class='hover-template'>" +
            //           //   "<img src='/image/classCard/" + editTemplateArr[x].ControlImg + "' style='width:100%;height:100%;'>" +
            //           //   "<i class='fa fa-close del-template-img' data-indexs='" + -1 + "'></i></div>"
            //           tHtml = `<div style="position:absolute;
            //           left:${nowHtml5.offsetLeft}px;
            //           top:${nowHtml5.offsetTop}px;
            //           width:${(nowHtml5.offsetWidth * 4 + 60)}px;
      ...
      windowResize () {
        ...
        this.offsetLeftBox = []
        var doms = document.getElementsByClassName('xy-drag-box')
        ...
        // 模板自适应
        var ysDom = $('.xy-drag-box'); var resizeDom = $('#templateModule>div')
        for (var k = 0; k < ysDom.length; k++) {
          if (resizeDom.length > 0) {
            for (var j = 0; j < resizeDom.length; j++) {
      ...
      drop (event) {
        ...
        // 结束拖拽后添加模板
        var dropDom = $('#templateModule'); var childDom = ''; var endDom = $('.xy-drag-box'); var rh1 = 1; var rh2 = 1; var rh3 = 1; var rh4 = 1; var rh5 = 1; var nowi = ''
        if (this.dragStartType.ControlWidth === 1 && this.dragStartType.ControlHeight === 1) {
      ...
      async addTemplate () {
        ...
        html2canvas(this.$refs.tamplateble, {
          ...
          var fileName = 'template' + '.jpeg'
          oMyForm.append('file', blob, fileName)
          oMyForm.append('BussinessStr', 0)
          this.savePingImg(fileName, blob, blob.size)
        })
      },
      // 上传屏保
