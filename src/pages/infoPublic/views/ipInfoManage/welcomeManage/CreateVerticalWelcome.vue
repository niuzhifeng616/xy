    ...
    <!-- 屏保管理-创建竖版 // 缺少面包屑 -->
      <div class="xy-content-title" style="display:flex;justify-content:space-between;align-items:center">
          <div>
              ...
              <Input maxLength="10" v-cloak v-model="welcomeName" style="width:300px;margin-right:10px;" />
    ...
    <div class="xy-content-body">
      <div style="display:flex;">
          <div style=" width: calc(100% - 337px);">
              ...
              <div class="welcome-bg-vertical" ref="tamplateble" id="tamplateble" style="border-radius:6px" v-cloak>
    ...
    mounted () {
      this.$nextTick(() => {
        ...
        $('.changeBox').keydown(function () {
          let postionBox = $(this).parents('.postionBox-vertical')
    ...
    methods: {
        ...
        if (res.success) {
          if (res.data.length > 0) {
            ...
            if (!isUpload) {
              ...
              strurl = strurl.replace('\\', '\\')
              strurl = strurl.replace('\\', '\\')
              this.welcomeBgExample = strurl// 默认显示图片
              document.getElementById('tamplateble').style.backgroundImage = 'url(' + this.welcomeBgExample.replace(/\\/g, '/') + ')'
      ...
      async submitCreat () {
        ...
        html2canvas(this.$refs.tamplateble, {
          ...
          this.oMyForm = new FormData()
          let fileName = 'template' + '.jpeg'
          this.oMyForm.append('file', blob, fileName)
          this.oMyForm.append('fileType', 'image')
          this.savePingImg(fileName, arr, blob, blob.size)
        })
      },
      // 上传屏保
      ...
      async UploadScreenSaverThumbnail (arr, adress) {
        let c = await this.xy.post(
          this.apiPath + `/api/Resource/CreateScreenSaver`, {
            ScreenSaverName: this.welcomeName,
            LatencyTime: this.latencyTime,
            ScreenMode: 1,
            BackgroundImage: this.welcomeBgExample,
            BackgroundDLink: '',
