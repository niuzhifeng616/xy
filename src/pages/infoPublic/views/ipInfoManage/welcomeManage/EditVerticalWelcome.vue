  ...
  <div class="xy-content-module" id="XyContentModuleReplace" @click="allClick">
    <!-- 屏保管理-创建竖版 // 缺少面包屑 -->
      <div class="xy-content-title" style="display:flex;justify-content:space-between;align-items:center">
          <div>
              <span style="color:rgba(0,0,0,0.6);">屏保名称：</span>
              <Input maxLength="10" v-cloak v-model="welcomeName" :disabled="row._disabled" style="width:300px;margin-right:10px;" />
              <span style="color:rgba(0,0,0,0.6);margin-left:24px;">等待时间：</span>
              <Select v-model="latencyTime" style="width:150px" :disabled="row._disabled">
              ...
              <DatePicker type="datetimerange" v-cloak :editable="false" v-model="times" :disabled="row._disabled"
                @on-change="timeChange" placement="bottom-end" placeholder="开始时间-结束时间"
                style="width: 300px;"></DatePicker>
          </div>
      </div>
    <div class="xy-content-body">
      <div style="display:flex;">
          <div style=" width: calc(100% - 337px);">
              <Button type="primary" icon="ios-add" @click="addTextBox">添加文本框</Button>
              <div class="welcome-bg-vertical" ref="tamplateble" id="tamplateble" :style="{borderRadius:'6px'}" v-cloak>
                  <div class="postionBox-vertical" :id="item.id" v-for="(item,index) in boxData" :key="index" :style="{'z-index':item.index,'top':item.yVal+'px','left':item.xVal+'px'}">
                      <div class="style-box">
                          <div>
                      ...
                      <!-- <Icon type="ios-expand" class="move-btn"></Icon> -->
          ...
          <div>
              <div style="display:flex;justify-content:space-between;align-items:center;height:40px;">
                  ...
                  <div>
                      <!-- <Upload :action='(apiPath + "/api/Resource/UploadScreenSaverImage?ScreenMode=1")'
                ...
                <ul class="welcome-bg-list-vertical">
                  <li v-for="(item, index) in imgList" :key="index">
                      <img :src="item.DownloadLink" v-cloak @click="changeBg(item)" />
                      <Icon type="ios-trash-outline" class="img-del-vertical" @click="delImg(item)" v-if="!item.IsDefault"></Icon>
      ...
      <div class="submit-fix-btn">
          <Button class="xy-btn-primary" shape="circle" :disabled="row._disabled" @click="submitCreat">保存</Button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.row = this.$store.state.infoPublic.item
      this.getScreenSaverByID()
      this.getAllScreenSaverImage()
    },
    mounted () {
      this.$nextTick(() => {
        ...
        $('#XyContentModuleReplace').on('mousedown', '.changeBox1', function (e) {
          ...
          let btnY = e.clientY
          let isMove = true
          postionBox = $(this).parents('.postionBox-vertical')
          maxHeight = $('.welcome-bg-vertical').innerHeight()
          maxWidth = $('.welcome-bg-vertical').innerWidth()
          ...
          $(this).mousemove(function (ve) {
            if (isMove) {
              ...
              if (btnMoveX !== 0) {
                if (btnMoveX > 0) { // 往右移动
                  if ((btnMoveX + pbLeft + postionBox.width()) < (maxWidth - 150)) {
    ...
    methods: {
      ...
      // 获取当前屏保信息
      async getScreenSaverByID () {
        this.xy.loading()
        let res = await this.xy.get(
          this.apiPath + `/api/Resource/GetScreenSaverByID`, {
            screenSaverID: this.row.ScreenSaverID
          }
        )
        this.xy.unloading()
        if (res.success) {
          let resData = res.data
          this.welcomeName = resData.ScreenSaverName
          this.latencyTime = String(resData.LatencyTime)
          this.times = [resData.StartDate, resData.EndDate]
          let strurl = resData.BackgroundDLink
          strurl = strurl.replace('\\\\', '\\\\')
          strurl = strurl.replace('\\', '\\')
          strurl = strurl.replace('\\', '\\')
          strurl = strurl.replace('\\', '\\')
          this.welcomeBgExample = resData.BackgroundImage
          let bg = document.getElementById('tamplateble')
          bg.style.backgroundImage = `url(${strurl})`
          this.boxData = []
          for (let i = 0; i < resData.Contents.length; i++) {
            var htmlArr = resData.Contents[i].TextContent.split('\\r\\n')
            var texthtml = ''
            for (var n = 0; n < htmlArr.length; n++) {
              texthtml += '<div>' + htmlArr[n] + '</div>'
            }
            this.boxData.push({
              id: 'textBox' + (i + 1),
              index: i + 1,
              ...
              colorVal: {
                key: resData.Contents[i].FontColor,
                val: this.colorData[resData.Contents[i].FontColor - 1].val
              },
              textContent: texthtml,
              fontSize: resData.Contents[i].FontSize,
              fontWeight: resData.Contents[i].IsBold,
              textBoxHeight: resData.Contents[i].TextBoxHeight,
              textBoxWidth: resData.Contents[i].TextBoxWidth,
              xVal: resData.Contents[i].XAxis,
              yVal: resData.Contents[i].YAxis,
              isShowMask: false
            })
          }
        } else {
      ...
      handleSuccess (res, file) {
        if (res.data.success) {
          ...
          this.BackgroundID = resData.ResourceID // 默认图片ID
          let bg = document.getElementById('tamplateble')
          bg.style.backgroundImage = `url(${this.apiPath}${strurl})`
          this.getAllScreenSaverImage(true)
        }
      },
      changeBg (item) {
      ...
      async submitCreat () {
        ...
        let arr = []
        let welcomeBgX = $('.welcome-bg-vertical').offset().left
        let welcomeBgY = $('.welcome-bg-vertical').offset().top
      ...
      async UploadScreenSaverThumbnail (arr, adress) {
        ...
        let c = await that.xy.post(
          that.apiPath + `/api/Resource/EditScreenSaver`, {
            ScreenSaverID: that.row.ScreenSaverID,
            ...
            StartDate: this.xy.moment(this.times[0]).format('YYYY-MM-DD  HH:mm:ss'),
            EndDate: this.xy.moment(this.times[1]).format('YYYY-MM-DD  HH:mm:ss'),
            Contents: arr
          }
        )
        that.xy.unloading()
