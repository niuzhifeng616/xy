          ...
          <div class="main-info">
              <div class="left">
                  <span class="avatar">
                      ...
                      <span v-else :title="infos.FullName"> {{ showName(infos.ApplicantName) }} </span>
                  </span>
                  <p class="name-number">
            ...
            <ul class="leave-list" style="margin-bottom: 66px;">
                ...
                <li class="info-item">
                    ...
                    <span class="content" style=" display: inline-block; vertical-align: top; width: 80%;">
                ...
                <li class="info-item img-list">
                  <span class="title">请假图片</span>
                   <!-- <span v-if="infos.Images.length == 0" class="content">无</span> -->
                    <div v-if="infos.Images && infos.Images.length > 0" style=" display: inline-block; vertical-align: top; width: 80%;">
            ...
            <div v-if="infos.ApplicationStateNew === 0 || infos.ApplicationStateNew === 1"
  ...
  export default {
    ...
    data () {
      return {
        ...
        apiPath: `${this.xyApi}/att`,
        base: `${this.xyApi}/base`
      }
    },
    computed: {
      row () {
    ...
    watch: {
      row: {
        handler (newVal) {
          this.LeaveApplicationID = newVal.BusinessID
          this.getrows(newVal)
        },
        immediate: true
      }
    ...
    methods: {
      ...
      async getrows () {
        this.xy.loading()
        let res = await this.xy.get(`${this.apiPath}/api/StudentLeave/GetStudentLeaveByID?LeaveApplicationID=${this.LeaveApplicationID}`)
        if (res.success) {
          this.xy.unloading()
          ...
          this.workflow = res.data.ApprovalFlowNodes
          this.ApprovalFlowNodes.forEach((element) => {
            ...
            if (element.Status === '待审批') {
              ...
              if (element.ApprovalFlowNodeName === '或签') {
                ...
                for (var c = 0; c < element.ApprovalMemberList.length; c++) {
                  // 循环当前节点下的所有人
                  console.log(element.ApprovalMemberList[c].Status)
              ...
              if (element.ApprovalFlowNodeName === '会签') {
                ...
                for (var a = 0; a < element.ApprovalMemberList.length; a++) {
                  // 循环当前节点下的所有人
              ...
              if (element.ApprovalFlowNodeName === '或签') {
                ...
                for (var s = 0; s < element.ApprovalMemberList.length; s++) {
                  // 循环当前节点下的所有人
                  // 找到当前用户  Status = 0 待审批, 2 已通过，3 已拒绝
