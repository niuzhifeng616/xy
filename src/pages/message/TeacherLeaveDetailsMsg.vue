  ...
  <div class="detailModel">
          ...
          <div class="main-info pr">
            <div class="left">
              ...
              <p class="name-number">
                ...
                <span class="schoolNumber"
                  >{{detailInfo.ApplicationStateNameNew}}</span
                >
              </p>
            </div>
        ...
        <ul class="leave-list">
          ...
          >
            ...
            <div
              class="demo-upload-list" v-for="(item, index) in detailInfo.LeavePictrueList" :key="index">
              <template>
                <div>
          ...
          >
            ...
            <i-input
              v-model="Option" :autosize="{minRows: 5,maxRows: 5}"
              type="textarea"
              placeholder="请输入审批意见"
              maxlength="100"
            />
        ...
        <div v-if="isFrom === 0 && (detailInfo.ApplicationStateNew === 0 || detailInfo.ApplicationStateNew === 1)"
          style="margin-top:20px;text-align: left;">
          <Button type="primary" @click="RevokedMine">撤销</Button>
        </div>
        <div v-if="isFrom === 1 && tableType==='approval' && detailInfo.JudgeCanOperation===true"
          style="margin-top:20px;text-align: left;">
  ...
  export default {
      ...
      row: {
        handler (newVal) {
          ...
          if (this.row.Subject === '教师请假申请') {
            ...
            this.isFrom = ''
            this.getDetailInfo(rows)
          } else if (rows.Subject === '教师请假申请已撤销') {
            this.isFrom = ''
            this.getDetailInfo(rows)
          }
        },
    ...
    methods: {
      async getDetailInfo (row) {
        this.xy.loading()
        let res = await this.xy.get(this.xyApi + `/att/api/TeacherLeave/GetDetailByID?id=${row.TeacherLeaveApplicationID}`, null)
        if (res.success) {
          // 详情区域上发显示
      ...
      RevokedMine () {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.xy.post(`${this.xyApi}/att/api/TeacherLeave/RevocationLeaveApplication`, params).then(res => {
              if (!res.success) {
                this.$Modal.error({
                  title: '撤销失败',
                  content: res.msg
      ...
      approvalRequire (value) {
        ...
        this.xy.post(`${this.xyApi}/att/api/TeacherLeave/SaveApprovalBranchStatus`, params).then(res => {
          if (!res.success) {
            this.xy.unloading()
            this.$Modal.error({
              title: '审批失败',
