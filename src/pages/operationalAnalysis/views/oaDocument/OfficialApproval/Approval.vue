<template>
  <div class="xy-content-module" id="XyContentModuleReplace">
      <div id="xy-initialize-breadcrumb" v-if="isInitial"></div>
      <div v-else id="GroupBreadcrumb" class="xy-crumbs clearfix">
          <div class="xy-crumbs-box fl">
              <span class="main">
                <a href="javascript:void(0)" class="xy-crumbs-name">主页</a>
              </span>
              <span class="pr">
                <span class="xy-crumbs-icon">/</span>
                <a data-auto="loadPageTemp" data-url="" data-index="1" data-pid="23" class="xy-crumbs-name" @click="ReturnClick">消息</a>
              </span>
              <span class="pr">
                <span class="xy-crumbs-icon">/</span>
                <a data-auto="loadPageTemp" data-url="" data-index="1" data-pid="23" class="xy-crumbs-name">消息处理</a>
              </span>
          </div>
      </div>
             ...
             <div class="mr-15">
                ...
                <i-select
                  ...
                  >
                    <i-option
                      v-cloak
                      v-for="types in approvalValue"
                      :value="types.approvalID"
                      :key="types.approvalID"
                      >
                       {{ types.approvalName }}
                    </i-option>
                </i-select>
      ...
      <div class="xy-content-body">
        ...
        >
          ...
          <!-- 审批类型 -->
          <div slot="OfficialType">
              公文审批
          </div>
          <!-- 摘要 -->
          <div slot="OfficialDocTheme" slot-scope="text,record"  style="min-width:350px">
      ...
      <Drawer v-model="mInfo" width="560" :transfer="false" :mask-closable="false">
          <div slot="header">
            <p>{{rows.FullName}}提交的公文</p>
          </div>
          <div class="drawer">
            <div id="DrawerDetails" v-if="mInfo">
                <!-- 右侧详情 -->
              <DetailsLook :row="rows"   @loadTable="loadTable" :type="type" :tableType="tableType"></DetailsLook>
            </div>
          </div>
      </Drawer>
  </div>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            width: 120,
            dataIndex: '',
            scopedSlots: { customRender: 'OfficialType' }
          },
          {
            title: '摘要',
    ...
    methods: {
      ...
      changeTableType: function (value) {
        this.pagination.current = 1
        this.tableType = value
        value === 'cc' ? this.getCCTableList() : this.loadTable()
      },
      getData () {
        // this.tableType = value
        this.tableType === 'cc' ? this.getCCTableList() : this.loadTable()
      },
      async getCCTableList () {
        ...
        let rs = await this.xy.get(this.$store.state.apiPath + '/api/OfficialDocument/GetCCRecordByPage', params)
        if (rs.success) {
      ...
      async loadTable () {
        ...
        let rs = await this.xy.get(this.$store.state.apiPath + '/api/OfficialDocument/GetApprovalRecordByPage', params)
        if (rs.success) {
          this.data = rs.data.data
          this.pagination.total = rs.data.totalRecords
        }
