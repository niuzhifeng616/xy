    ...
    <div class="detail">
      <div class="detail-top">
        <div class="person clearfix">
          <span class="left fl" v-if="detail.Icon === ''">{{
            xy.showName(detail.FullName)
          }}</span>
          <img
            v-else
        ...
        <div class="officeStatus">
          ...
          />
           <img
            ...
            class="no-data-img"
            src="@/assets/common/attendance/seal-undo.png"
          />
        </div>
      </div>
      <div class="detail-con">
        ...
        <div v-if="detail.IsArchive">
          <span>归档日期：</span>
          <div v-cloak>
            {{this.xy.moment(detail.LastModifyByDate).format('YYYY-MM-DD HH:mm:ss')}}
            <!-- {{
              this.xy.date.format(
                new Date(detail.LastModifyByDate),
                "yyyy-MM-dd HH:mm"
              )
            }} -->
          </div>
      ...
      <div
        class="detail-approval"
        v-if="coper != 'cc' && detail.JudgeCanOperation === true"
      >
       <Form ref="caseForm"  label-position="top" class="edit-form">
          <FormItem label="审批意见" prop="caseName" labelFor="approvalValue">
            <i-input v-model="approvalValue" elementId="approvalValue" type="textarea" maxlength="100"></i-input>
          </FormItem>
       </Form>
      </div>
      <div
      ...
      >
        ...
        <Button
          ...
          @click="approvalFun(3)"
          :loading="isShowSpin"
          style="margin-right:26px;min-width:90px;height:36px"
        >
          拒绝
        </Button>
      </div>
...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      if (_this.type === 0) {
        // _this.info = xy.param.getValues('MessageModel')
    ...
    methods: {
      ...
      getRows: function () {
        var _this = this
        _this.coper = _this.tableType
        this.officeId = this.row.OfficialDocID
        this.getDetail()
      },
      // 明细
      getDetail: function () {
            ...
            data.ApprovalFlow.forEach(function (element, index) {
              ...
              element.ApprovalMemberList.forEach(function (memberItem, memberIndex) {
                ...
                if (!element.isFinished) {
                  // 获取当前需审批的节点：第一个非完成节点
                  if (
                    _this.approvalData.ApprovalFlowNodeID === undefined &&
      ...
      xyFormatDate: function (date, format) {
        ...
        // return date ? xy.date.format(date, format) : ''
      ...
      approvalFun: function (type) {
        ...
        // console.log(type)
        let params = {
          ApprovalFlowID: _this.detail.ApprovalFlowID,
          ApprovalFlowNodeID: _this.approvalData.ApprovalFlowNodeID,
          Option: _this.approvalValue,
          Status: type,
          OfficialDocID: _this.officeId
        }
        _this.xy.post(`${_this.$store.state.apiPath}/api/OfficialDocument/SaveApprovalBranchStatus`, params).then(res => {
          this.xy.unloading()
          if (res.success) {
            if (_this.type === 0) {
              _this.mInfo = false
