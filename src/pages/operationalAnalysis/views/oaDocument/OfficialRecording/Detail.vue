...
<div id="DrawerDetails">
    ...
    <div class="detail">
        <div class="detail-top">
            <div class="person clearfix">
              ...
              <img v-else style="width: 50px;height: 50px;border-radius: 50%;float:left;" :src="detail.Icon" alt="">
                <div>
                    <div class="name" v-cloak>{{detail.FullName}}</div>
        ...
        <div class="detail-con">
            <div>
                <span>主题：</span>
                <div v-cloak>{{detail.OfficialDocTheme}}</div>
            </div>
            <div>
                <span>公文标题：</span>
                <div v-cloak>{{detail.OfficialDocTitle}}</div>
            </div>
            <div>
                <span>公文种类：</span>
                <div v-cloak>{{detail.OfficialDocTypeName}}</div>
            </div>
            <div>
                <span>秘密等级：</span>
                <div v-cloak>{{detail.OfficialDocSecretLevelName}}</div>
            </div>
            <div>
                <span>紧急程度：</span>
                <div v-cloak>{{detail.OfficialDocEmergencyLevelName}}</div>
            </div>
            <div>
                <span>发文机关：</span>
                <div v-cloak>{{detail.FromOffice}}</div>
            </div>
            <div>
                <span>发文字号：</span>
                <div v-cloak>{{detail.IssueNumber}}</div>
            </div>
            <div>
                <span>发文内容：</span>
                <div>
                    <div class="office-info" v-for="(item,index) in detail.List" :key="index">
                        <span class="nowrap" v-cloak>{{item.FileName}}</span>
            ...
            <div>
                <span>附注：</span>
                <div v-cloak>{{detail.Remark}}</div>
            </div>
            <!--已归档展示归档日期-->
            <div v-if="detail.IsArchive">
                <span>归档日期：</span>
                 <div v-cloak>{{this.xy.moment(detail.LastModifyByDate).format('YYYY-MM-DD HH:mm:ss')}}</div>
                <!-- <div v-cloak>{{xy.date.format(new Date(detail.LastModifyByDate), 'yyyy-MM-dd HH:mm')}}</div> -->
            </div>
        </div>
        <!-- <span class="title" style="color: #999;">审批流程</span> -->
        <WorkflowApproval  :workflow="workflow"></WorkflowApproval>
        <div class="bottom-btn" v-show="detail.Status == 2 && !detail.IsArchive || detail.IsArchive">
            <!--审批通过且未归档显示“归档”-->
            <!--<i-button v-if="detail.Status == 2 && !detail.IsArchive" type="primary" class="xy-primary"
                @click="fileFun()" :loading="isShowSpin" style="margin-right:30px;">归档</i-button>-->
          <Button
            class="xy-btn-primary"
            v-if="detail.Status == 2 && !detail.IsArchive"
            shape="circle"
            @click="fileFun()"
            :loading="isShowSpin"
          ...
          <!--已归档展示销毁按钮-->
          <Button
            v-if="detail.IsArchive"
            class="xy-btn-primary"
            shape="circle"
            @click="destructionFun()"
            :loading="isShowSpin"
            style="margin-right:30px;"
          >
            销毁
          </Button>
            <!--<i-button v-if="detail.IsArchive" type="primary" class="xy-primary" @click="destructionFun()"
                :loading="isShowSpin" style="margin-right:30px;">销毁</i-button>-->
        </div>
    </div>
</div>
</template>
<script>
  ...
  export default {
    components: { WorkflowApproval },
    props: ['type', 'MessageModel'],
    data: function () {
      return {
        workflow: [],
        isShowSpin: false,
    ...
    created: function () {
      this.mInfo = true
      var _this = this
      // _this.type = xy.param.getValues('type')
      // console.log(_this.type)
      if (_this.type === 0) {
        // _this.info = this.xy.param.getValues('MessageModel')
        _this.info = _this.MessageModel
        console.log(_this.info)
        _this.ServiceDoMainName = _this.info.ServiceDoMainName
        _this.WebDoMainName = _this.info.WebDoMainName
        _this.officeId = _this.info.BusinessID
        _this.getDetail()
      } else {
        _this.ServiceDoMainName = this.xy.subMenuServiceUrl
        _this.WebDoMainName = this.xy.WebDoMainName
        _this.getRows()
      }
    },
    // watch: {
    ...
    methods: {
      ...
      getRows: function () {
        ...
        // _this.params = xy.param.getValues('row')
        _this.params = _this.MessageModel
        _this.officeId = _this.params.OfficialDocID
        _this.approvalFlowId = _this.params.ApprovalFlowID
        _this.getDetail()
        // }
      },
      // 明细
      getDetail: function () {
        ...
        // xy.http.get(xy.string.format('{0}/api/OfficialDocument/GetByID', _this.ServiceDoMainName), {
        //   ID: _this.officeId
        //   // ApprovalFlowID: _this.approvalFlowId
        // }, function (res) {
        //   xy.unloading()
        //   if (res.data.success) {
        //     var data = res.data.data
        //     var lastPass_node = 0 // 临时记载：当前遍历中，最后通过审批的节点索引下标；其中，第0个为发起人，即尚未有审批
        //     data.currentAuditNodeIndex = -1 // 当前待审批节点的下标
        //     data.ApprovalFlow.forEach(function (element, index) {
        //       console.log(element)
        //       // 缩略预览模式开关
        ...
        //       var lastTime_member = undefined // 暂存：最近的一次时间点
        //       element.ApprovalMemberList.forEach(function (memberItem, memberIndex) {
        //         // 缩略图列表仅显示前4个，其余显示省略号；若共5人，则不显示省略号，直接全部显示
        //         if (memberIndex < 4 || element.ApprovalMemberList.length === 5) {
        //           element.overviewList.push(memberItem)
        ...
        //           } else if (element.ApprovalFlowNodeName === '会签') {
        //             // 记录最后一次会签时间
        //             if (lastTime_member === undefined) {
        //               // 记载首次审批的时间
        //               lastTime_member = memberItem.ApprovalDate
        //             } else {
        //               var lastTime = new Date(lastTime_member)
        //               var thisTime = new Date(memberItem.ApprovalDate)
        //               if (thisTime > lastTime) {
        //                 element.lastComplete_time = memberItem.ApprovalDate
        //                 element.lastComplete_status = memberItem.Status
        ...
        //   } else {
        //     xy.message.error(_this, res.data.msg)
        //   };
        // })
      },
      // 展开或折叠
      ...
      fileFun: function () {
        ...
        _this.$Modal.confirm({
          title: '归档',
          content: '<p>您确定要将此公文归档吗？</p>',
          onOk: () => {
            _this.isShowSpin = true
            _this.xy.post(`${_this.$store.state.apiPath}/api/OfficialDocument/Archive?id=${_this.officeId}`).then(res => {
              _this.isShowSpin = false
              if (res.success) {
                _this.xy.msgSuc('归档成功。')
        ...
        // xy.model.confirm(this, {
        //   title: '归档',
        //   content: '<p>您确定要归档此公文吗？</p>',
        //   onOk: function () {
        //     _this.isShowSpin = true
        //     xy.http.post(xy.string.format('{0}/api/OfficialDocument/Archive?id={1}', _this.ServiceDoMainName, _this.officeId), null, function (res) {
        //       _this.isShowSpin = false
        //       if (res.data.success) {
        //         xy.message.success(_this, '归档成功')
        //         if (_this.type == 0) {
        //           _this.mInfo = false
        //         } else {
        //           _this.ReturnClick()
      ...
      // 销毁
      destructionFun: function () {
        var _this = this
        _this.$Modal.confirm({
          title: '销毁',
          content: '<p>您确定要销毁此公文吗？</p>',
          onOk: () => {
            _this.isShowSpin = true
            _this.xy.post(`${_this.$store.state.apiPath}/api/OfficialDocument/Delete?id=${_this.officeId}`).then(res => {
              _this.isShowSpin = false
              if (res.success) {
                _this.xy.msgSuc('销毁成功。')
                // xy.message.success(_this, '收回成功')
                _this.ReturnClick()
              }
            })
