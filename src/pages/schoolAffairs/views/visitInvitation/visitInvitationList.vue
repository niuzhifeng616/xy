<template>
  <div class='xy-content-module'>
    <div class='xy-content-title xy-flex xy-between xy-center'>
        <div class="fl">
            <Button class="xy-primary" @click="createLeave">发起邀请</Button>
        </div>
        <div class="fr pr">
            ...
            <div class="fl xy-content-title-search">
                <Input search v-model.trim="searchTxt" @on-search="themeSubmit" placeholder="请输入主题" style="width: 260px;" />
            </div>
        </div>
    </div>
    <div class='xy-content-body'>
      <!-- 这里是table等DOM -->
        <XyAntTable
          ...
          :scrollx="'max-content'"
        >
          <div slot="InvatationTitle" style="min-width:200px" slot-scope="text,record">
    ...
    methods: {
      ...
      async loadTable () {
        ...
        let params = {
          ...
          invatationTitle: this.searchTxt, // 邀请主题
          invatationArriveTime: this.defaultDate ? this.defaultDate : '', // 邀请到访时间
          invatationUser: '' // 受邀人
        }
        let res = await this.xy.get(
          this.xyApi + '/vst' + '/invatation/GetMyInvatationPagelist', params
      ...
      // 发起邀请-跳转页面
      createLeave () {
        this.$store.commit('common/getThreeMenuName', '发起邀请')
        this.$router.push({
